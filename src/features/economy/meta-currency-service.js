export const META_CURRENCIES = Object.freeze([
  "voidShards",
  "bossCores",
  "anomalyData",
  "challengeSeals",
...
export function createMetaCurrencyService(saveStore) {
  async function transact(entries, source) {
    return saveStore.update((save) => {
      for (const [currency, delta] of Object.entries(entries)) {
        if (!META_CURRENCIES.includes(currency))
  ...
  return {
    transact,
    credit(currency, amount, source) {
      if (amount < 0) throw new Error("Credit must be positive");
      return transact({ [currency]: amount }, source);
    },
    debit(currency, amount, source) {
