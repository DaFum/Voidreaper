...
const requirementValue = (tags, requirement) =>
  tags.totals.get(requirement.id) ?? 0;
...
function resolveSynergies(tags, definitions, context = {}) {
  ...
  for (const definition of definitions) {
    ...
    const missing = requirements.filter(
      (requirement) =>
        requirementValue(tags, requirement) < (requirement.minimum ?? 1),
    );
    const forbiddenBy = (definition.blockedBy ?? []).filter(
      (tagId) => (tags.totals.get(tagId) ?? 0) > 0,
    );
    const contextBlocked =
      (definition.minimumCorruption !== undefined &&
        (context.corruption ?? 0) < definition.minimumCorruption) ||
      (definition.minimumLoad !== undefined &&
        (context.loadRatio ?? 0) < definition.minimumLoad);
    if (forbiddenBy.length || contextBlocked) {
      ...
      missing.length === 1 &&
      requirementValue(tags, missing[0]) >= (missing[0].minimum ?? 1) - 1
    ) {
...
export function createTagEngine(definitions, synergyDefinitions) {
  ...
  return {
    collect(sources) {
      ...
      for (const id of tags.totals.keys())
        if (!knownTags.has(id)) console.warn(`Unknown tag: ${id}`);
      return tags;
    },
    resolve(tags, context) {
      return resolveSynergies(tags, synergyDefinitions, context);
    },
