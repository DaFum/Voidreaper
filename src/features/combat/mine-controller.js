export function createMineController({ maximum = 16 } = {}) {
  ...
  const removeZone = (run, mine) => {
    const zones = run?.zones;
    if (!zones || typeof zones.indexOf !== "function" || !mine) return;
    const index = zones.indexOf(mine);
  ...
  return {
    place(context, payload = {}) {
      ...
      const mine = {
        id: context.run.ids.create("mine"),
        x: context.player.x,
        y: context.player.y,
        age: 0,
        armed: false,
      ...
      mines.push(mine);
      context.run.zones.push(mine);
      return mine;
    },
    update(_context, dt) {
      for (const mine of mines) {
        mine.age += dt;
        mine.armed = mine.age >= 0.45;
      }
    },
    detonate(context, multiplier = 1) {
      for (const mine of mines.splice(0)) {
        removeZone(context.run, mine);
        context.emitEffect({
          id: "spawn-zone",
          payload: {
            ...mine,
            damage: (mine.damage ?? 18) * multiplier,
            radius: 54,
          },
        });
      }
