...
function createEmptyLoadout({ secondRelicUnlocked = false } = {}) {
  return {
    slots: Object.fromEntries(
      Object.entries(LOADOUT_SLOT_LAYOUT).map(([slot, count]) => [
        slot,
        Array.from(
          {
            length:
              slot === EQUIPMENT_SLOT.RELIC && !secondRelicUnlocked ? 1 : count,
          },
          () => null,
        ),
      ]),
...
export function createStarterLoadout() {
  ...
  loadout.slots[EQUIPMENT_SLOT.SHIP][0] = {
    instanceId: "starter-vesper",
    definitionId: "vesper",
  };
  loadout.slots[EQUIPMENT_SLOT.PRIMARY_WEAPON][0] = {
    instanceId: "starter-railgun",
    definitionId: "railgun",
  };
  loadout.slots[EQUIPMENT_SLOT.REACTOR][0] = {
    instanceId: "starter-standard-core",
    definitionId: "standard-core",
  };
  return loadout;
}

export function resolvePrimaryLoadout(metaSave) {
  return metaSave?.loadouts?.primary?.slots
    ? metaSave.loadouts.primary
    : createStarterLoadout();
}

export function deriveEquipmentCatalogEntries(
  definitions,
  { isUnlocked = () => true, loadout = null } = {},
) {
  ...
  return definitions.map((definition) => {
    ...
    const state = equippedSlots.length
      ? "equipped"
      : unlocked
        ? "available"
        : "locked";
  ...
  return {
    equip(loadout, slot, index, item) {
      if (
        !loadout.slots[slot] ||
        index < 0 ||
        index >= loadout.slots[slot].length
      )
        throw new Error(`Unknown loadout slot: ${slot}[${index}]`);
      const definition = registry.require(item.definitionId);
      if (definition.slot !== slot)
        throw new Error(`${definition.id} cannot be equipped in ${slot}`);
      if (!unlocks.isUnlocked(definition))
        throw new Error(`${definition.id} is not unlocked`);
      const currentlyEquipped = loadout.slots[slot][index];
      if (
        definition.unique &&
        sources(loadout).some(
          (source) =>
            source.id === definition.id &&
            source.instanceId !== item.instanceId &&
            source.instanceId !== currentlyEquipped?.instanceId,
        )
      ) {
    ...
    inspect(loadout) {
      ...
      for (let i = 0; i < equipped.length; i++) {
        ...
        if (!reactor && source.slot === EQUIPMENT_SLOT.REACTOR)
          reactor = source;
        ...
        startingCorruption +=
          source.item?.corruptionLevel ?? source.corruption ?? 0;
      }

      const capacity =
        (reactor?.energyCapacity ?? 0) + (ship?.energyCapacity ?? 0);
