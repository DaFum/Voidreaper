...
export function createCorruptionState(value = 0) {
  ...
  return {
    ...
    tier: tier.id,
    bookings: [],
    discoveredSignatures: new Set(),
    committedTransformations: new Set(),
  };
}
  ...
  return {
    change(state, amount, sourceId, runTime = 0, { allowAbyss = false } = {}) {
      ...
      const bookingIndex = state.bookings.findIndex(
        (booking) => booking.sourceId === sourceId,
      );
      const previousBooking = state.bookings[bookingIndex];
      const booking = Object.freeze({
        sourceId,
        amount: (previousBooking?.amount ?? 0) + amountChanged,
        at: runTime,
      });
      ...
      if (tier.id !== state.tier) {
        ...
        state.tier = tier.id;
        if (CORRUPTION_SIGNATURES[tier.id])
          state.discoveredSignatures.add(CORRUPTION_SIGNATURES[tier.id]);
        eventBus?.emit("corruption-threshold", {
          tier: tier.id,
          previousTier,
          value: state.value,
          rule: tier,
      ...
      eventBus?.emit("corruption-changed", {
        previous,
        value: state.value,
    ...
    summary(state) {
      ...
      for (const booking of state.bookings)
        sources.set(
          booking.sourceId,
          (sources.get(booking.sourceId) ?? 0) + booking.amount,
        );
      return {
        value: state.value,
        tier: state.tier,
        sources: [...sources.entries()],
        signatures: [...state.discoveredSignatures],
      };
    },
  };
}
