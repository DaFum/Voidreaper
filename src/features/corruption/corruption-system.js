...
export function createCorruptionSystem({ eventBus } = {}) {
  return {
    change(state, amount, sourceId, runTime = 0, { allowAbyss = false } = {}) {
      ...
      state.bookings.push(Object.freeze({ sourceId, amount: state.value - previous, at: runTime }));
      ...
      eventBus?.emit("corruption-changed", { previous, value: state.value, amount: state.value - previous, sourceId });
      return state.value;
    },
