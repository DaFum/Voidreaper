...
export function createHeatSystem({ eventBus, modules } = {}) {
  return {
    add(state, amount, sourceId = "unknown") {
      state.crossedOverheat =
        state.crossedOverheat ||
        (state.value < 100 && state.value + amount >= 100);
      state.value += amount;
      state.coolingDelay = Math.max(state.coolingDelay, 0.65);
      state.sourceHeat.set(
        sourceId,
        (state.sourceHeat.get(sourceId) ?? 0) + amount,
      );
    },
    update(
      state,
      dt,
      { coolingRate, generationMultiplier = 1, canExceed = false } = {},
    ) {
