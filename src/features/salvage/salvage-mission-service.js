...
export function createSalvageMissionService(saveStore) {
  return {
    ...
    async apply(mission, result) { return saveStore.update(save => { const signal = save.wreckSignals[mission.signalId]; if (!signal || signal.status === "recovered") return save; signal.status = "recovered"; signal.result = result; const item = structuredClone(signal.itemSnapshot); if (result === "complete") save.inventory[item.instanceId] = item; if (result === "damaged") save.inventory[item.instanceId] = { ...item, stability: 45, durability: 45 }; if (result === "corrupted") save.inventory[item.instanceId] = { ...item, corruptionLevel: Math.max(75, item.corruptionLevel ?? item.corruption ?? 0) }; if (result === "blueprint") save.blueprints[item.definitionId ?? item.id] = { sourceSignalId: signal.id }; return save; }); }
  };
}
