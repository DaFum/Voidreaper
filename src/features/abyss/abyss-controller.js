...
export function createAbyssController(eventBus) {
  return {
    enter(run) { run.mode = "abyss"; run.campaign.abyssDepth = 1; run.corruption.maximum = Infinity; eventBus?.emit("abyss-entered", {}); return this.profile(run.campaign.abyssDepth); },
    ...
    advance(run) { run.campaign.abyssDepth += 1; run.corruption.value += 4 + run.campaign.abyssDepth; return this.profile(run.campaign.abyssDepth); },
