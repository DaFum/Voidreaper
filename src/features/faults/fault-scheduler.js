...
function calculateFaultPressure({
  loadTier,
  heat,
  corruption,
  stability = 100,
...
export function createFaultScheduler({ rng, profiles, faults, eventBus }) {
  ...
  return {
    ...
    update(now, inputs, components = []) {
      state.pressure = calculateFaultPressure({
        ...inputs,
        cooldown: now - state.lastAt,
      });
      ...
      const candidates = components.filter(
        (component) =>
          component.faultProfileId && (component.disabledUntil ?? 0) <= now,
      );
      const component = candidates.length
        ? rng.pick(candidates)
        : { id: "system", faultProfileId: "standard" };
      const profile =
        profileMap.get(component.faultProfileId) ?? profileMap.get("standard");
      const tier = tierFor(state.pressure);
      const faultId = rng.pick(
        profile[tier]?.length ? profile[tier] : profile.light,
      );
