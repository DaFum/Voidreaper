...
export function createMerchantService({ modules = [], weapons = [], reactors = [], currencyService, eventBus } = {}) {
  ...
  return {
    ...
    sell(run, item) { const before = run.inventory.length; run.inventory = run.inventory.filter(entry => entry !== item); if (run.inventory.length === before) return false; currencyService.award(run, "sale", Math.max(1, Math.floor(merchantPrice(item) / 4))); return true; },
