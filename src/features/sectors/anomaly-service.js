...
export function createAnomalyService(eventBus) {
  return {
    select(seed, seenIds = []) {
      const pool = ANOMALY_EVENTS.filter(
        (event) => !seenIds.includes(event.id),
      );
      const rng = createRunRng(seed);
      return (pool.length ? pool : ANOMALY_EVENTS)[
        rng.integer(0, (pool.length ? pool : ANOMALY_EVENTS).length - 1)
      ];
    },
    resolve(run, anomalyEvent, choiceId) {
      const choice = anomalyEvent.choices.find(
