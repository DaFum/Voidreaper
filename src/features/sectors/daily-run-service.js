...
export function createDailyRunService({
  ...
  return {
    config(value = new Date()) {
      ...
      return {
        date,
        seed: hashString(`${date}:${seedVersion}:${contentVersion}`),
        seedVersion,
        contentVersion,
        normalized: true,
      };
    ...
    apply(run, config) {
      run.seed = config.seed >>> 0;
      run.rng = createRunRng(config.seed);
      run.mode = "daily";
      run.daily = config;
      run.permanentBonusesDisabled = true;
      run.prototypeBonusesDisabled = true;
      return run;
    },
    async record(config, score) {
      await saveStore?.update((save) => {
        save.legacy.dailyBest[config.date] = Math.max(
          score,
          save.legacy.dailyBest[config.date] ?? 0,
        );
      });
    },
  };
