...
export function createPrototypeVault(save, { capacity = 20 } = {}) {
  ...
  return {
    ...
    filter(filters = {}) { return values().filter(item => (!filters.family || item.family === filters.family) && (!filters.tag || item.tags?.some(tag => (tag.id ?? tag) === filters.tag)) && (!filters.rarity || item.rarity === filters.rarity) && (!filters.stability || describeStability(item) === filters.stability) && (!filters.source || item.source === filters.source)); },
