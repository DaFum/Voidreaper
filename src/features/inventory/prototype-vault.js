export function createPrototypeVault(save, { capacity = 20 } = {}) {
  ...
  return {
    get capacity() { return Math.min(100, capacity + (save.unlocks["vault-30"] ? 10 : 0) + (save.unlocks["vault-50"] ? 20 : 0) + (save.unlocks["vault-100"] ? 50 : 0)); },
    add(item) { const target = values().length < this.capacity ? vault : overflow; target[item.instanceId] = structuredClone(item); return target === vault ? "vault" : "overflow"; },
    ...
    repair(id, fragments = 1) { const item = vault[id]; if (!item || save.currencies.salvageFragments < fragments) return false; save.currencies.salvageFragments -= fragments; item.stability = "stable"; item.durability = 100; return true; },
