export function createBuildHistoryService(saveStore) {
  return {
    capture(run, result) { return { id: `build-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`, createdAt: new Date().toISOString(), ship: run.loadout?.shipId, weapon: run.loadout?.weaponId, reactor: run.loadout?.reactorId, modules: [...(run.loadout?.moduleIds ?? [])], evolutions: run.build.evolutions.map(entry => entry.id ?? entry), tags: [...((run.build.tags?.totals ?? run.build.tags) ?? new Map()).entries()], seed: run.seed, result, favorite: false }; },
