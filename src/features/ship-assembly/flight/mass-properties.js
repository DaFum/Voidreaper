...
export function calculateMassProperties(
  nodes,
  rootPosition = DEFAULT_ROOT_POSITION,
) {
  let totalMass = 0;
  let comX = 0;
  let comY = 0;
...
export function mapToFlightModifiers(properties, thrust = {}) {
  return {
    ...
    turnMultiplier: clamp(
      1 - properties.rotationalInertia / 900000,
      0.85,
      1.08,
    ),
    dodgeDistanceMultiplier: clamp(
      1 + (thrust.dodgeAuthority ?? 0) / 100 - properties.totalMass / 520,
      0.8,
      1.15,
    ),
    dodgeCooldownMultiplier: clamp(
      1 + properties.totalMass / 650 - (thrust.lateral ?? 0) / 130,
      0.85,
      1.25,
    ),
    driftBias: clamp(properties.lateralImbalance / 900, -0.04, 0.04),
    recoilControl: clamp((thrust.structuralSupport ?? 0) / 100, 0, 0.35),
  };
}
