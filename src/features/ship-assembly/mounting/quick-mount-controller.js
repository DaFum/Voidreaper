export function createQuickMountController({
  ...
  return {
    ...
    open(pendingMount, context) {
      if (session) close();
      const suggestions = refresh(context);
      if (!suggestions.length) {
        runInventory.store(pendingMount.itemInstanceId);
        pendingMountService.defer(pendingMount.pendingMountId);
        return { opened: false, reason: "no-compatible-port" };
      }
      session = {
        pendingMount,
        context,
      ...
      pendingMountService.setSuggestions(
        pendingMount.pendingMountId,
        suggestions.map((item) => item.portId),
      );
      timeScaleService?.push(
        "quick-mount",
        context.settings?.pauseOnMount ? 0 : 0.12,
      );
    ...
    select(index) {
      if (session && index >= 0 && index < session.suggestions.length)
        session.selectedIndex = index;
      return session;
    },
    confirm() {
      if (!session) return { mounted: false, reason: "closed" };
      const snapshot = assemblyService.getSnapshot();
      if (snapshot.structuralRevision !== session.assemblyRevision) {
        session.suggestions = refresh(session.context);
        ...
        if (!session.suggestions.length) {
          ...
          close();
          return { mounted: false, reason: "no-compatible-port" };
        }
      }
      const suggestion = session.suggestions[session.selectedIndex],
        nodeId = commandService.mount({
          ...
          expectedRevision: session.assemblyRevision,
        });
    ...
    defer() {
      if (!session) return { deferred: false };
      runInventory.store(session.pendingMount.itemInstanceId);
      pendingMountService.defer(session.pendingMount.pendingMountId);
      close();
