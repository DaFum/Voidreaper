...
const boundsFor = zone => {
  const p = zone.transform.position, shape = zone.shape, radius = shape.radius ?? shape.outerRadius ?? 0;
  let s = radius || 24;
  if (shape.length != null) {
    ...
    for (let i = 0; i < shape.points.length; i++) {
      ...
      const mx = Math.max(Math.abs(pt.x), Math.abs(pt.y));
      if (mx > m) m = mx;
    }
    s = m;
  }
