...
export const rotationForPortDirection = (direction = { x: 0, y: 1 }) =>
  Math.atan2(direction.y, direction.x) - Math.PI / 2;

export function resolvePortWorldTransform(port, geometrySnapshot) {
  const parent = geometrySnapshot?.nodes?.find(
    (node) => node.nodeId === port.parentNodeId,
  );
  const localPosition = port.localPosition ?? {
  ...
  return {
    position: add(
      ...
      rotate(localPosition, parent.worldRotation ?? 0),
    ),
