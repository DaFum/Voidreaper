...
export function traceLauncherDoor(c,x,y,w,h,open=0){c.beginPath();c.roundRect(x-w/2-open*w*.3,y-h/2,w,h,2)}
export function traceCoolingFin(c,x,y,length,angle=0){c.save();c.translate(x,y);c.rotate(angle);traceTaperedPlate(c,{length,frontWidth:5,rearWidth:1,notch:2});c.restore()}
export function traceThrusterNozzle(c,x,y,size){c.beginPath();c.moveTo(x-size,y-size*.6);c.lineTo(x+size,y-size*.35);c.lineTo(x+size,y+size*.35);c.lineTo(x-size,y+size*.6);c.closePath()}
export function traceShieldRing(c,x,y,r,gap=.35){c.beginPath();c.arc(x,y,r,gap,Math.PI-gap);c.moveTo(x-r*Math.cos(gap),y-r*Math.sin(gap));c.arc(x,y,r,Math.PI+gap,Math.PI*2-gap)}
export function traceBrokenPlateEdge(c,length,seed=1){c.beginPath();c.moveTo(0,0);for(let i=1;i<=5;i++)c.lineTo(length*i/5,(i%2?1:-1)*(2+(seed+i)%4));}
