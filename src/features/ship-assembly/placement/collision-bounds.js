...
// where the structural spine is rendered longer (2.25) than standard modules (1.45).
// The size table and factors are duplicated here (rather than imported) to avoid a
...
const lengthFactor = rendererId => rendererId === "core-structural-spine" ? 2.25 : 1.45;
export function boundsFromCenter(center = { x: 0, y: 0 }, size, ownerId, rendererId) { const half = (MODULE_SIZE[size] ?? MODULE_SIZE.M) * (lengthFactor(rendererId) / 2 + RADIUS_FACTOR); return { ownerId, minX:center.x-half,minY:center.y-half,maxX:center.x+half,maxY:center.y+half }; }
