...
export function createTriggerEngine({
  ...
  let stepDepth = 0;
  let stepEffects = 0;
  const subscriptions = TRIGGER_EVENTS.map((eventName) =>
    eventBus.on(eventName, (payload) => process(eventName, payload)),
  );

  function process(eventName, payload = {}) {
    if (stepDepth >= 12 || stepEffects >= 100) {
      console.warn(
        `Trigger budget exceeded at ${eventName}; remaining chain discarded.`,
      );
      return;
    }
    stepDepth += 1;
    try {
      for (const trigger of triggers) {
        ...
        if (
          trigger.condition &&
          !trigger.condition(payload.context ?? payload, payload)
        )
          continue;
        ...
        for (const effect of trigger.effects ?? []) {
          ...
          stepEffects += 1;
          effects.execute(
            { ...effect, sourceId: effect.sourceId ?? trigger.sourceId },
  ...
  return {
    setSources(sources) {
      triggers = sources.flatMap((source) =>
