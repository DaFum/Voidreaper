export function createArchitectController(eventBus) {
  return {
    ...
    chooseFinal(state, choice, run) { if (!['stabilize', 'overload'].includes(choice)) return false; state.finalChoice = choice; if (choice === "stabilize") run.corruption.value = Math.max(0, run.corruption.value - 25); else { run.player.energy.ratio = Math.max(1.5, run.player.energy.ratio); run.heat.value = 100; } return true; },
