...
export function createArchitectController(eventBus) {
  return {
    ...
    chooseFinal(state, choice, run) { if (!["stabilize", "overload"].includes(choice)) return false; state.finalChoice = choice; if (choice === "stabilize") changeRunCorruption(run, -25, "architect-stabilize"); else { run.player.energy.ratio = Math.max(1.5, run.player.energy.ratio); run.services.heat.add(run.heat, 100 - run.heat.value, "architect-overload"); } return true; },
