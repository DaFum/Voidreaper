...
const adapter = {
  ...
  update(context, state, dt) {
    ...
    if (state.spawnCooldown <= 0) {
      state.controller.spawn(context, "drone-core");
      state.spawnCooldown = 4;
    }
    state.controller.update(context, dt);
  },
  fire(context, state, target) {
    if (!target) return false;
    for (const drone of state.controller.drones)
      context.emitEffect(
        {
          id: "spawn-projectile",
          payload: { x: drone.x, y: drone.y, damage: 7, speed: 500 },
        },
        { target },
      );
    return state.controller.drones.length > 0;
  },
  onEquip: () => {},
  onUnequip: () => {},
  getTelemetry: (_context, state) => ({
    activeDrones: state.controller.drones.length,
  }),
};
...
export default createWeaponDefinition(
  {
    ...
    effects: [{ id: "summon-unit" }, { id: "spawn-projectile" }],
    faultProfileId: "weapon-drone",
