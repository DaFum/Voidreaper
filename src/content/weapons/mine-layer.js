...
const adapter = {
  ...
  update(context, state, dt) {
    ...
    if (
      state.cooldown <= 0 &&
      context.player.vx * context.player.vx +
        context.player.vy * context.player.vy >
        25
    ) {
      state.controller.place(context, { damage: 22 });
      state.cooldown = 1.1;
    }
  },
  fire: () => false,
  ...
  getTelemetry: (_context, state) => ({
    activeMines: state.controller.mines.length,
  }),
};
...
export default createWeaponDefinition(
  {
    ...
    tags: [
      { id: "Weapon", value: 1 },
      { id: "Mine", value: 3 },
      { id: "Explosive", value: 2 },
      { id: "Control", value: 2 },
      { id: "Cooldown", value: 1 },
    ],
    effects: [{ id: "spawn-zone" }],
    faultProfileId: "weapon-projectile",
