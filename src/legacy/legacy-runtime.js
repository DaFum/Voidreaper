...
const AudioSys = {
  ...
  shoot() {
    this.blip(880 + Math.random() * 100, 0.07, "square", 0.05, 240);
  },
  laser() {
    this.blip(1400, 0.12, "sawtooth", 0.06, 300);
  },
  hit() {
    this.noise(0.07, 0.1, 1400);
  },
  kill() {
    this.noise(0.2, 0.18, 500);
    this.blip(160, 0.16, "sawtooth", 0.09, 40);
  },
  hurt() {
    this.blip(200, 0.25, "sawtooth", 0.22, 60);
    this.noise(0.2, 0.18, 300);
  },
  gem() {
    this.blip(1200 + Math.random() * 300, 0.1, "sine", 0.08, 2000);
  },
  shard() {
    this.blip(1800, 0.2, "triangle", 0.1, 2600);
  },
  pickup() {
    [700, 1050, 1400].forEach((f, i) =>
      setTimeout(() => this.blip(f, 0.14, "triangle", 0.12), i * 60),
    );
  },
  freeze() {
    this.blip(2200, 0.7, "sine", 0.14, 400);
    this.noise(0.5, 0.08, 3000);
  },
  bomb() {
    this.noise(0.6, 0.3, 250);
    this.blip(70, 0.6, "sawtooth", 0.22, 25);
  },
  levelup() {
    [523, 659, 784, 1046].forEach((f, i) =>
      setTimeout(() => this.blip(f, 0.25, "triangle", 0.14), i * 90),
    );
  },
  evolve() {
    [392, 523, 659, 784, 1046, 1318].forEach((f, i) =>
      setTimeout(() => this.blip(f, 0.3, "triangle", 0.15), i * 80),
    );
    this.noise(0.8, 0.1, 900);
  },
  streak() {
    [880, 1174, 1568].forEach((f, i) =>
      setTimeout(() => this.blip(f, 0.16, "square", 0.1), i * 55),
    );
  },
  event() {
    this.blip(330, 0.5, "square", 0.1, 660);
  },
  boss() {
    this.blip(80, 1.2, "sawtooth", 0.25, 30);
    this.noise(0.8, 0.15, 200);
  },
  combo(n) {
    this.blip(600 + n * 120, 0.08, "square", 0.07, 900 + n * 150);
  },
  gameover() {
    [400, 300, 200, 100].forEach((f, i) =>
      setTimeout(() => this.blip(f, 0.5, "sawtooth", 0.18, f * 0.5), i * 180),
    );
  },
};
...
resize();
const Bloom = createBloomPass({ strength: 0.42, blur: 4 });
...
class Pool {
  ...
  releaseAll() {
    while (this.live.length) this.items.push(this.live.pop());
  }
}
...
const Input = {
  ...
  dx: 0,
  dy: 0,
  el: document.getElementById("stick") ?? document.createElement("div"),
  knob: document.getElementById("knob") ?? document.createElement("div"),
  init() {
  ...
  onDown(e) {
    ...
    this.el.style.left = this.sx - 62 + "px";
    this.el.style.top = this.sy - 62 + "px";
...
const META = [
  ...
  },
  {
    id: "mreroll",
    nm: "Signal Scrambler",
    ds: "+1 upgrade reroll per run / lv",
    max: 4,
    cost: (l) => 60 + l * 50,
  },
  {
    id: "mbanish",
    nm: "Void Censor",
    ds: "+1 upgrade banish per run / lv",
    max: 3,
    cost: (l) => 60 + l * 50,
  },
  {
    id: "mrevive",
    nm: "Phoenix Sigil",
    ds: "+1 revive at 50% hull per run / lv",
    max: 2,
    cost: (l) => 200 + l * 250,
  },
  {
...
const UPGRADES = [
  ...
  },
  {
    ...
    ico: "✺",
    nm: "Nova Pulse",
    ds: "periodic shockwave (faster/lv)",
    max: 5,
    apply: (p) => {
      p.nova++;
      p.novaCd = Math.max(2.2, 6 - p.nova);
    },
  },
  {
    id: "bspeed",
...
const EVOLUTIONS = LEGACY_EVOLUTIONS.map((definition) => ({
  ...
  req: (definition.requirements ?? [])
    .filter((requirement) => requirement.type === "upgrade")
    .map((requirement) => ({
      id: requirement.id,
...
const ACHIEVEMENTS = [
  ...
  },
  {
    id: "evo1",
    nm: "TRANSCEND",
    ds: "unlock an evolution",
    shards: 40,
    test: (g) => g.evolutions > 0,
  },
  {
    id: "c10",
    nm: "UNBROKEN",
  ...
  },
  {
    id: "boss3",
    nm: "KINGSLAYER",
    ds: "kill 3 bosses in one run",
    shards: 120,
    test: (g) => g.bossKills >= 3,
  },
  {
    id: "lv15",
    nm: "ASCENDANT",
    ds: "reach level 15",
    shards: 60,
    test: (g) => g.player && g.player.lvl >= 15,
  },
];
...
const mkShock = () => ({
  ...
  life: 0,
  maxLife: 0.5,
  color: FORGED_ABYSS_PALETTE.energy,
  dead: true,
});
...
const Game = {
  ...
  metaLv(id) {
    return Persist.data.meta[id] || 0;
  },
  grand(a, b) {
    return a + this.grng() * (b - a);
  },
  gpick(arr) {
    return arr[(this.grng() * arr.length) | 0];
  },
  ...
  reset(mode) {
    ...
    const seed =
      mode === "daily"
        ...
        : mode === "tutorial"
          ? hashStr("VR-TUTORIAL")
          : hashStr(
              `${Date.now()}-${performance.now()}-${crypto.getRandomValues(new Uint32Array(1))[0]}`,
            );
    this.seed = seed;
    ...
    for (let i = 0; i < 260; i++)
      this.stars.push({
        x: (this.rng() * 2 - 1) * this.arena * 1.4,
        y: (this.rng() * 2 - 1) * this.arena * 1.4,
        z: 0.3 + this.rng() * 0.7,
        s: 0.6 + this.rng() * 1.8,
        hue: palette[(this.rng() * palette.length) | 0],
        tw: this.rng() * TAU,
        tws: 0.5 + this.rng() * 2.5,
      });
    ...
    this.rerolls = m("mreroll");
    this.banishes = m("mbanish");
    this.revives = m("mrevive");
  ...
  quit() {
    if (this.mode !== "tutorial") this.bankShards();
    this.state = "menu";
    UI.menu();
  },
  ...
  startWave(n) {
    ...
    if (n >= 12) {
      ...
      if (n === 12) {
        UI.eventBanner("▼ CORRUPTION RISING ▼", "#ff2d78");
        AudioSys.event();
      }
    }
  ...
  spawnEnemy(type, x, y, immediate) {
    ...
    const e = {
      ...
      dmg: t.dmg * (1 + this.corruption * 0.4),
      xp: t.xp,
      color: t.color,
      score: t.score,
      shape: t.shape,
      ...
      warps: !!t.warps,
      healer: !!t.healer,
      warpT: this.grand(2, 4),
      healT: 2,
      hitT: 0,
      fireT: this.grand(1, 2.5),
      wobble: this.grand(0, TAU),
      elite: null,
      volatile: false,
      vampiric: false,
      orbCd: 0,
  ...
  spawnFromBudget() {
    ...
    for (const regionalType of externalRegionRosterProvider?.() ?? []) {
      if (regionalType !== "boss" && ETYPES[regionalType])
        roster.push(regionalType);
    }
  ...
  triggerEvent() {
    ...
    switch (ev.id) {
      ...
      case "rift": {
        const a = this.grand(0, TAU),
          d = 300;
        this.eventData.x = clamp(
          p.x + Math.cos(a) * d,
          -this.arena + 100,
          this.arena - 100,
        );
        this.eventData.y = clamp(
          p.y + Math.sin(a) * d,
          -this.arena + 100,
          this.arena - 100,
        );
    ...
    switch (this.event.id) {
      case "meteor": {
        ...
        if (this.eventData.t <= 0) {
          ...
          z.x = p.x + this.grand(-320, 320);
          z.y = p.y + this.grand(-320, 320);
          z.r = 54;
          z.life = z.maxLife = 1.1;
          z.dps = 0;
          z.color = "#ff8f1f";
          z.pull = 0;
          z.telegraph = true;
      ...
      case "rift": {
        ...
        if (this.eventData.t <= 0) {
          ...
          const a = this.grand(0, TAU);
          this.spawnEnemy(
            this.gpick(["swarm", "chaser", "orbiter"]),
            this.eventData.x + Math.cos(a) * 40,
            this.eventData.y + Math.sin(a) * 40,
            true,
          );
  ...
  fire(p) {
    ...
    p.angle = base;
    const n = p.shots,
      spread = n > 1 ? (p.evoPrism ? 0.07 : 0.13) * (n - 1) : 0;
    for (let i = 0; i < n; i++) {
      const a =
        base -
        spread / 2 +
        (n > 1 ? (spread * i) / (n - 1) : 0) +
        this.grand(-0.02, 0.02);
    ...
    mf.x = p.x + Math.cos(base) * 18;
    mf.y = p.y + Math.sin(base) * 18;
    mf.vx = Math.cos(base) * 60;
    mf.vy = Math.sin(base) * 60;
    mf.life = mf.maxLife = 0.08;
    mf.size = 7;
    mf.color = p.evoPrism ? "#fff6c9" : "#c9fff0";
    mf.drag = 0.8;
  ...
  hurtPlayer(p, dmg) {
    ...
    this.shockwave(p.x, p.y, 130, "#ff2d78", 0.4);
  ...
  damageEnemy(e, dmg, isCrit, fromAngle) {
    ...
    if (e.shielded && fromAngle !== undefined) {
      ...
      let diff = Math.abs(
        ((((fromAngle - facing + Math.PI) % TAU) + TAU) % TAU) - Math.PI,
      );
      if (diff > 2.2) {
    ...
    if (isCrit && this.player.evoReaper) {
      ...
      this.reaperBuf = this.reaperBuf || [];
      this.hash.query(e.x, e.y, R, this.reaperBuf);
      for (const o of this.reaperBuf)
        if (o !== e && dist2(e.x, e.y, o.x, o.y) < R * R) {
          o.hp -= dmg * 0.4;
          o.hitT = 0.08;
  ...
  killEnemy(e) {
    ...
    // killstreak callouts
    if (
      this.streakIdx < STREAKS.length &&
      this.combo >= STREAKS[this.streakIdx][0]
    ) {
    ...
    if (e.boss) this.shockwave(e.x, e.y, 340, "#ffd166", 0.8);
    else if (e.r > 18) this.shockwave(e.x, e.y, 90, e.color, 0.4);
    this.hitStop = Math.max(
      this.hitStop,
      e.boss ? 0.12 : e.r > 18 ? 0.045 : 0.02,
    );
    AudioSys.kill();
    ...
    for (let g = 0; g < gems; g++) {
      ...
      gem.x = e.x + this.grand(-14, 14);
      gem.y = e.y + this.grand(-14, 14);
      gem.vx = this.grand(-60, 60);
      gem.vy = this.grand(-60, 60);
      gem.val = Math.max(1, Math.round(e.xp / gems));
      gem.t = 0;
      gem.shard = false;
    }
    ...
    if (this.grng() < shardChance) {
      ...
      for (let s = 0; s < n; s++) {
        const gem = this.gems.get();
        gem.x = e.x + this.grand(-10, 10);
        gem.y = e.y + this.grand(-10, 10);
        gem.vx = this.grand(-50, 50);
        gem.vy = this.grand(-50, 50);
        gem.val = 1;
        gem.t = 0;
        gem.shard = true;
      }
    }
    if (e.elite && this.grng() < 0.35 + this.metaLv("mluck") * 0.04)
      this.dropPickup(e.x, e.y);

    if (e.splits && !e.wasSplit) {
      for (let s = 0; s < 3; s++) {
        const c = this.spawnEnemy(
          "swarm",
          e.x + this.grand(-16, 16),
          e.y + this.grand(-16, 16),
          true,
        );
        c.wasSplit = true;
      }
  ...
  usePickup(pk) {
    ...
    switch (pk.type.id) {
      case "heal":
        ...
        this.burst(p.x, p.y, 20, "#06ffa5", 200);
        break;
      case "bomb":
        this.bombBlast(p.x, p.y, 320, 60 * p.dmgMul);
        AudioSys.bomb();
        this.shake(16);
        break;
      case "freeze":
  ...
  bombBlast(x, y, R, dmg) {
    this.ring(x, y, R);
    this.shockwave(x, y, R * 1.15, "#ff8f1f", 0.5);
  ...
  rollOptions() {
    ...
    for (const ev of EVOLUTIONS) {
      ...
      if (
        ev.req.length &&
        ev.req.every((r) => (this.upgradeCounts[r.id] || 0) >= r.minimum)
      ) {
        opts.push({ evo: ev });
        break;
      }
    ...
    const avail = UPGRADES.filter(
      (u) =>
        (this.upgradeCounts[u.id] || 0) < u.max && !this.banished.has(u.id),
    );
    const src = avail.slice();
    while (opts.length < 3 && src.length)
      opts.push({ up: src.splice((this.grng() * src.length) | 0, 1)[0] });
    return opts;
  },

  levelUp() {
    if (externalLevelUpGate && !externalLevelUpGate(this)) {
      this.deferredLevelUps = (this.deferredLevelUps || 0) + 1;
      return;
    }
    AudioSys.levelup();
  ...
  novaBlast(p) {
    ...
    this.shake(6);
    AudioSys.noise(0.35, 0.25, 700);
    this.ring(p.x, p.y, R);
    this.shockwave(p.x, p.y, R * 1.1, p.evoSing ? "#c77dff" : "#4cc9f0", 0.45);
    const snap = getArrayFromPool([]);
  ...
  bankShards() {
    ...
    const gain = Math.round(this.shardsRun * (1 + this.metaLv("mshard") * 0.1));
  ...
  burst(x, y, n, color, spd) {
    for (let i = 0; i < n; i++) {
      ...
      const a = Math.random() * TAU,
        v = spd * 0.2 + Math.random() * spd * 0.8;
      pt.x = x;
      pt.y = y;
      pt.vx = Math.cos(a) * v;
      pt.vy = Math.sin(a) * v;
      pt.life = pt.maxLife = 0.3 + Math.random() * 0.55;
      pt.size = 1.5 + Math.random() * 2.5;
      pt.color = color;
      pt.drag = 0.92;
    }
  },
  implode(x, y, color, r) {
    for (let i = 0; i < 10; i++) {
      const pt = this.parts.get();
      const a = Math.random() * TAU,
        d = r * 2 + Math.random() * r * 2;
      pt.x = x + Math.cos(a) * d;
      pt.y = y + Math.sin(a) * d;
      pt.vx = -Math.cos(a) * d * 6;
      pt.vy = -Math.sin(a) * d * 6;
      pt.life = pt.maxLife = 0.16;
      pt.size = 2;
      pt.color = color;
      pt.drag = 1;
    }
  },
  shockwave(x, y, maxR, color = FORGED_ABYSS_PALETTE.energy, duration = 0.5) {
  ...
  ring(x, y, R) {
    for (let i = 0; i < 36; i++) {
      const pt = this.parts.get();
      const a = (i / 36) * TAU;
      pt.x = x + Math.cos(a) * 20;
      pt.y = y + Math.sin(a) * 20;
      pt.vx = Math.cos(a) * R * 2.4;
      pt.vy = Math.sin(a) * R * 2.4;
      pt.life = pt.maxLife = 0.4;
      pt.size = 3;
      pt.color = FORGED_ABYSS_PALETTE.glow;
      pt.drag = 0.88;
    }
  },
  floatText(x, y, txt, color, size, heavy) {
    ...
    t.txt = txt;
    t.color = color;
    t.life = t.maxLife = heavy ? 1.3 : 0.9;
    t.size = size || 12;
    t.heavy = !!heavy;
  },
  shake(m) {
    if (!isReducedMotion()) this.cam.shake = Math.max(this.cam.shake, m);
  },
  glitch() {
    document.body.classList.add("glitching", "hitfx");
    clearTimeout(this.glitchTimer);
    this.glitchTimer = setTimeout(
      () => document.body.classList.remove("glitching", "hitfx"),
      220,
    );
  },
  ...
  gameOver() {
    ...
    const best =
      this.mode === "daily"
        ? Persist.data.dailyBest[todayKey()] || 0
        : Persist.data.best;
    const isBest = this.mode === "tutorial" ? false : this.score > best;
    if (this.mode === "daily") {
      if (isBest) Persist.data.dailyBest[todayKey()] = this.score;
  ...
  step(dt) {
    ...
    if (
      (this.deferredLevelUps || 0) > 0 &&
      (!externalLevelUpGate || externalLevelUpGate(this))
    ) {
      this.deferredLevelUps--;
      this.levelUp();
      return;
    }
    const p = this.player;
    ...
    for (let i = this.enemies.length - 1; i >= 0; i--) {
      ...
      if (e.type === "orbiter" && dl < 240) {
        ...
        mvx = mvx * 0.35 + (-dyp / dl) * 0.9 * s;
        mvy = mvy * 0.35 + (dxp / dl) * 0.9 * s;
      }
      if (e.ranged && dl < 260 && !e.boss) {
        mvx *= -0.6;
        mvy *= -0.6;
      }
      ...
      if (e.fusing !== false) {
        e.fusing -= dt;
        if (e.fusing <= 0) {
          this.burst(e.x, e.y, 26, "#ff8f1f", 300);
          AudioSys.bomb();
          this.shake(6);
          if (dist2(e.x, e.y, p.x, p.y) < 110 * 110) this.hurtPlayer(p, e.dmg);
          this.killEnemyQuiet(e);
          continue;
        }
      }
      ...
      if (e.boss && !frozen) {
        ...
        if (e.bossKind === "hive" && e.phaseT > 3.5) {
          ...
          for (let k = 0; k < 4; k++) {
            const c = this.spawnEnemy(
              "swarm",
              e.x + this.grand(-40, 40),
              e.y + this.grand(-40, 40),
              true,
            );
            c.wasSplit = true;
          }
        ...
        if (e.bossKind === "serpent") {
          ...
          if (e.dashing > 0) {
            ...
            if (Math.random() < dt * 20) {
              ...
              pt.vx = 0;
              pt.vy = 0;
              pt.life = pt.maxLife = 0.5;
              pt.size = e.r * 0.7;
              pt.color = "#c77dff";
              pt.drag = 1;
            }
          } else if (e.dashT <= 0) {
            e.dashT = this.grand(2.5, 3.8);
            e.dashing = 0.55;
            // ⚡ Bolt: Avoid Math.atan2 and cos/sin for vector normalization in hot path
            const dist = Math.sqrt(dxp * dxp + dyp * dyp);
            if (dist > 0) {
              const f = (e.spd * 7) / dist;
              e.vx = dxp * f;
              e.vy = dyp * f;
            }
      ...
      if (!dashLock) {
        e.vx = lerp(
          e.vx,
          (mvx + sx * 1.4) * e.spd * eSlow,
          clamp(6 * dt, 0, 1),
        );
        e.vy = lerp(
          e.vy,
          (mvy + sy * 1.4) * e.spd * eSlow,
          clamp(6 * dt, 0, 1),
        );
      }
      e.x += e.vx * dt * (frozen && !dashLock ? 0 : 1);
      e.y += e.vy * dt * (frozen && !dashLock ? 0 : 1);

      if (e.ranged && !frozen) {
        e.fireT -= dt;
        if (e.fireT <= 0 && dl < 520) {
          e.fireT = e.boss ? 0.8 : this.grand(1.6, 2.6);
          this.enemyFire(e, p);
        }
      }
      const cr = e.r + p.r;
  ...
  updateProjectilesAndFX(dt, frozen) {
    ...
    this.bullets.update((b) => {
      ...
      for (const e of this.qbuf) {
        ...
        if (dist2(b.x, b.y, e.x, e.y) < rr * rr) {
          ...
          if (b.prism && hpBefore > 0 && e.hp <= 0)
            this.refract(e.x, e.y, b.dmg);
          const pt = this.parts.get();
          pt.x = b.x;
          pt.y = b.y;
          pt.vx = this.grand(-90, 90);
          pt.vy = this.grand(-90, 90);
          pt.life = pt.maxLife = 0.2;
          pt.size = 2;
          pt.color = "#efeaf7";
          pt.drag = 0.9;
    ...
    this.gems.update((g) => {
      ...
      if (d2p < mr * mr) {
        ...
        g.vx += ((p.x - g.x) / d) * pull * dt * 8;
        g.vy += ((p.y - g.y) / d) * pull * dt * 8;
      }
      g.vx *= 0.92;
      g.vy *= 0.92;
      g.x += g.vx * dt;
      ...
      if (d2p < (p.r + g.r + 4) * (p.r + g.r + 4)) {
        ...
        if (g.shard) {
          this.shardsRun++;
          AudioSys.shard();
          this.floatText(p.x, p.y - 24, "+1◇", "#4cc9f0", 11);
    ...
    this.pickups.update((pk) => {
      ...
      if (dist2(pk.x, pk.y, p.x, p.y) < (p.r + 16) * (p.r + 16)) {
        pk.dead = true;
        this.usePickup(pk);
      }
    });
  ...
  drawHexFloor() {
    ...
    const s = 64,
      ...
      vstep = s * Math.sqrt(3);
    const A = this.arena;
    const x0 = Math.max(-A, this.cam.x - W / 2 - s * 2),
      x1 = Math.min(A, this.cam.x + W / 2 + s * 2);
    const y0 = Math.max(-A, this.cam.y - H / 2 - s * 2),
      y1 = Math.min(A, this.cam.y + H / 2 + s * 2);
    ...
    cx.lineWidth = 1;
    cx.beginPath();
    const ci = Math.floor(x0 / hstep),
      cj0 = Math.floor(y0 / vstep);
    for (let i = ci; i * hstep < x1; i++) {
      ...
      for (let j = cj0 - 1; j * vstep < y1; j++) {
        ...
        for (let k = 0; k < 6; k++) {
          ...
          if (k === 0)
            cx.moveTo(
              cxx + Math.cos(a1) * s * 0.94,
              cyy + Math.sin(a1) * s * 0.94,
            );
          cx.lineTo(
            cxx + Math.cos(a2) * s * 0.94,
            cyy + Math.sin(a2) * s * 0.94,
          );
        }
      }
    }
  ...
  drawShip(p) {
    ...
    cx.shadowBlur = 0;
    if (p.shield > 0) {
      cx.strokeStyle = "#4f6df5";
      cx.shadowColor = "#4f6df5";
      cx.shadowBlur = 14;
      cx.lineWidth = 1.5;
      cx.beginPath();
      cx.arc(p.x, p.y, p.r + 8 + Math.sin(this.time * 4) * 2, 0, TAU);
      cx.stroke();
      cx.shadowBlur = 0;
    }
  },
  ...
  drawWorldBackdrop(p, camX, camY, t, W, H, shakeX, shakeY) {
    ...
    if (envHandled) {
      ...
      bg.addColorStop(0, "#070212");
      bg.addColorStop(0.6, "#04010a");
      bg.addColorStop(1, "#060113");
      cx.fillStyle = bg;
      cx.fillRect(0, 0, W, H);
    ...
    const A = this.arena;
    cx.strokeStyle = "rgba(255,45,120,.75)";
    cx.lineWidth = 3;
    cx.shadowColor = "#ff2d78";
    cx.shadowBlur = 20;
    cx.strokeRect(-A, -A, A * 2, A * 2);
    cx.setLineDash([14, 10]);
    cx.strokeStyle = "rgba(255,45,120,.3)";
    cx.lineWidth = 1;
    ...
    if (this.event && this.event.id === "rift") {
      const rx = this.eventData.x,
        ry = this.eventData.y;
      cx.strokeStyle = "#c77dff";
      cx.shadowColor = "#c77dff";
      cx.shadowBlur = 22;
      cx.lineWidth = 2;
      for (let k = 0; k < 3; k++) {
        cx.globalAlpha = 0.5 - k * 0.13;
        cx.beginPath();
        cx.arc(rx, ry, 26 + k * 14 + Math.sin(t * 3 + k) * 5, 0, TAU);
        cx.stroke();
      }
      cx.globalAlpha = 1;
      cx.shadowBlur = 0;
  ...
  drawWorldEntities(p, camX, camY, t, frozen) {
    ...
    for (const z of this.zones.live) {
      ...
      if (z.telegraph) {
        ...
        cx.beginPath();
        cx.arc(z.x, z.y, z.r * (1 - f), 0, TAU);
        cx.fill();
      } else {
        const rg = cx.createRadialGradient(z.x, z.y, 0, z.x, z.y, z.r);
        rg.addColorStop(0, z.color + "33");
        rg.addColorStop(1, z.color + "00");
        cx.fillStyle = rg;
        cx.globalAlpha = 0.6 * f + 0.2;
        cx.beginPath();
        cx.arc(z.x, z.y, z.r, 0, TAU);
        cx.fill();
    ...
    for (const pk of this.pickups.live) {
      const bob = Math.sin(pk.t * 4) * 4,
        blink = pk.life < 4 && ((pk.life * 6) | 0) % 2 === 0;
      if (blink) continue;
      cx.save();
      cx.translate(pk.x, pk.y + bob);
      cx.rotate(Math.sin(pk.t * 2) * 0.1);
      cx.strokeStyle = pk.type.color;
      cx.shadowColor = pk.type.color;
      cx.shadowBlur = 16;
      cx.lineWidth = 1.5;
    ...
    for (const b of this.bullets.live) {
      const c = b.prism
        ? FORGED_ABYSS_PALETTE.energySoft
        : FORGED_ABYSS_PALETTE.energy;
      cx.save();
    ...
    // enemies
    const hw = W / 2 + 120,
      hh = H / 2 + 120; // 120 is max enemy radius padding
    ...
    if (p.orbitals > 0) {
      const oR = p.evoHalo ? 78 : 62;
      cx.strokeStyle = p.evoHalo
        ? "rgba(255,45,120,.3)"
        : "rgba(76,201,240,.25)";
      cx.lineWidth = 1;
      cx.setLineDash([4, 6]);
      cx.beginPath();
      cx.arc(p.x, p.y, oR, 0, TAU);
      cx.stroke();
      cx.setLineDash([]);
      for (let i = 0; i < p.orbitals; i++) {
        const a = p.orbA + (i / p.orbitals) * TAU;
  ...
  drawParticlesAndFX(p, camX, camY, shakeX, shakeY, W, H, eclipse) {
    ...
    const rules = getRegionRules(
      this.visualRegionId ?? "shattered-approach",
      this.time,
    );
    const regionDark = Math.min(0.6, (1 - (rules?.visibility ?? 1)) * 1.9);
    const darkness = Math.max(eclipse ? 0.74 : 0, regionDark);
    ...
    const fxHandled =
      externalCombatFxRenderer?.capture({
        parts: this.parts.live,
        shocks: this.shocks.live,
        camX,
        camY,
        shakeX,
        shakeY,
    ...
    if (!fxHandled) {
      ...
      for (const s of this.shocks.live) {
        ...
        cx.globalAlpha = (1 - f) * 0.8;
        cx.lineWidth = 2 + (1 - f) * 4;
        cx.beginPath();
        cx.arc(s.x, s.y, s.maxR * ease, 0, TAU);
        cx.stroke();
        cx.globalAlpha = (1 - f) * 0.3;
        cx.lineWidth = 1.5;
        cx.beginPath();
        cx.arc(s.x, s.y, s.maxR * ease * 0.68, 0, TAU);
        cx.stroke();
      }
      cx.globalAlpha = 1;
      ...
      for (const pt of this.parts.live) {
        ...
        if (sparks && pt.vx * pt.vx + pt.vy * pt.vy > 2400) {
          ...
          cx.beginPath();
          cx.moveTo(pt.x - pt.vx * 0.035, pt.y - pt.vy * 0.035);
          cx.lineTo(pt.x, pt.y);
          cx.stroke();
        } else if (sparks) {
          drawBloomDot(cx, {
  ...
  drawHUDOverlaysAndPostFX(p, camX, camY, W, H, darkness, frozen) {
    ...
    for (const tx of this.texts.live) {
      ...
      const scale = tx.heavy
        ? 1 + (1 - f) * 0.15 + (f > 0.85 ? (f - 0.85) * 3 : 0)
        : 1;
      cx.globalAlpha = f;
      cx.font = `${tx.heavy ? "italic " : ""}700 ${(tx.size * scale) | 0}px 'Chakra Petch','Courier New',monospace`;
      cx.fillStyle = tx.color;
      cx.shadowColor = tx.color;
      cx.shadowBlur = tx.heavy ? 14 : 6;
    ...
    if (darkness > 0) {
      if (this.bloomOn !== false) {
        const offX = W / 2 - camX + this.cam.sx,
          offY = H / 2 - camY + this.cam.sy;
        const lights = [{ x: p.x, y: p.y, radius: 200, intensity: 1 }];
        const addLights = (list, limit, radius, intensity) => {
          for (let i = 0; i < list.length && i < limit; i++)
            lights.push({ x: list[i].x, y: list[i].y, radius, intensity });
        };
        addLights(this.bullets.live, 24, 46, 0.75);
        addLights(this.ebullets.live, 16, 40, 0.55);
        addLights(this.gems.live, 14, 28, 0.5);
        addLights(this.pickups.live, 8, 48, 0.7);
        for (const z of this.zones.live)
          if (!z.telegraph)
            lights.push({ x: z.x, y: z.y, radius: z.r, intensity: 0.5 });
        for (const s of this.shocks.live)
          lights.push({
            x: s.x,
            y: s.y,
            radius: s.maxR * (1 - s.life / s.maxLife) + 40,
            intensity: 0.7 * (s.life / s.maxLife),
          });
        if (this.boss)
          lights.push({
            x: this.boss.x,
        ...
        cx.beginPath();
        cx.rect(0, 0, W, H);
        const lx = W / 2 + (p.x - this.cam.x),
          ly = H / 2 + (p.y - this.cam.y);
        cx.arc(lx, ly, 170, 0, TAU, true);
        cx.fill("evenodd");
        const rim = cx.createRadialGradient(lx, ly, 140, lx, ly, 190);
        rim.addColorStop(0, "rgba(79,109,245,.14)");
        rim.addColorStop(1, "rgba(79,109,245,0)");
        cx.fillStyle = rim;
        cx.beginPath();
        cx.arc(lx, ly, 190, 0, TAU);
        cx.fill();
        cx.restore();
      }
    }
  ...
  loop(ts) {
    ...
    if (this.bloomOn) {
      if (this.frameCost > 28) this.bloomOn = false;
    ...
    const fxPresented =
      externalCombatFxRenderer?.present({ bloomOn: this.bloomOn }) === true;
...
const UI = {
  ...
  show(name) {
    ...
    if (name === "pausescr" || name === "over")
      this.el(name).querySelector("button")?.focus();
  },
  menu() {
    this.show("start");
    ...
    if (!Game.stars.length) {
      const rng = mulberry32(7),
        palette = ["#4318b8", "#4cc9f0", "#8b30e8", "#efeaf7", "#c77dff"];
      for (let i = 0; i < 260; i++)
        Game.stars.push({
          x: (rng() * 2 - 1) * 1960,
          y: (rng() * 2 - 1) * 1960,
    ...
    const b = Persist.data.best,
      db = Persist.data.dailyBest[todayKey()] || 0;
    this.el("best0").textContent =
      (b > 0 ? `BEST ${b}` : "") +
      (db > 0 ? ` · DAILY ${db}` : "") +
      (Persist.data.totalKills > 0
        ? ` · ${Persist.data.totalKills} TOTAL KILLS`
        : "");
    this.renderHangar();
    document.body.classList.remove("corrupted", "lowhp");
  ...
  hud(p, g) {
    ...
    if (this._lastHp !== hp) {
      this.el("hp-t").textContent = hp;
      this._lastHp = hp;
    }

    const hpPct = clamp(p.hp / p.maxHp, 0, 1);
    if (this._lastHpPct !== hpPct) {
      this.el("hpbar").querySelector("i").style.transform = `scaleX(${hpPct})`;
      this._lastHpPct = hpPct;
    }

    if (this._lastLvl !== p.lvl) {
      this.el("lvl-t").textContent = p.lvl;
      this._lastLvl = p.lvl;
    }

    const xpPct = clamp(p.xp / p.xpNext, 0, 1);
    if (this._lastXpPct !== xpPct) {
      this.el("xpbar").firstElementChild.style.transform = `scaleX(${xpPct})`;
      this._lastXpPct = xpPct;
    }

    if (this._lastWave !== g.wave) {
    ...
    if (this._lastKills !== g.kills) {
      this.el("kill-t").textContent = g.kills;
      this._lastKills = g.kills;
    }
    if (this._lastShards !== g.shardsRun) {
      this.el("shard-t").textContent = g.shardsRun;
      this._lastShards = g.shardsRun;
    }

    const tStr = fmtTime(g.time);
    if (this._lastTime !== tStr) {
      this.el("time-t").textContent = tStr;
      this._lastTime = tStr;
    }
  },
  shield(n) {
  ...
  bossHp(f) {
    this.el("bossfill").style.transform = `scaleX(${clamp(f, 0, 1)})`;
  },
  flash() {
  ...
  levelup(opts) {
    ...
    const rerollTitle =
      Game.rerolls <= 0
        ? "Reroll mutation choices - No rerolls remaining"
        : "Reroll mutation choices";
    ...
    const banishTitle =
      Game.banishes <= 0
        ? "Permanently remove a mutation - No banishes remaining"
  ...
  gameOver(g, shardsGained, isBest) {
    ...
    this.el("finalshards").textContent =
      `+${shardsGained} ◇ VOID SHARDS (TOTAL ${Persist.data.shards})`;
    this.el("best1").textContent =
      isBest && g.score > 0
        ? "▲ NEW BEST"
        : "BEST " +
          (g.mode === "daily"
...
export const legacyRuntime = Object.freeze({
  ...
  start() {
    ...
    UI.el("dailybtn").addEventListener("click", () => {
      AudioSys.unlock();
      AudioSys.resume();
      Game.start("daily");
    });
    UI.el("retrybtn").addEventListener("click", () => {
      AudioSys.resume();
    ...
    UI.el("quitbtn").addEventListener("click", () => {
      uiConfirm(
        "Are you sure you want to abandon this run? All progress will be lost.",
        {
          title: "ABANDON RUN",
    ...
    UI.el("banishbtn").addEventListener("click", () => {
      ...
      UI.el("banishbtn").style.background = Game.banishMode
        ? "rgba(255,45,120,.15)"
        : "";
      UI.toast(
        Game.banishMode ? "TAP A CARD TO BANISH IT" : "BANISH CANCELLED",
      );
    });
