...
export function renderConnector(ctx, connector, palette, { lod="high", energyFlow=0, layer="all" }={}) {
  ...
  if(layer!=="static"&&lod!=="low")drawEnergyRail(ctx,{from:connector.cable.from,to:connector.cable.to,color:palette.energy,width:Math.max(1,connector.spine.width*.14),flow:energyFlow});
  ctx.restore();
}
