function tracePath(ctx, path) {
  ...
  if (path.kind === "line") { ctx.moveTo(path.from.x, path.from.y); ctx.lineTo(path.to.x, path.to.y); ctx.lineWidth = path.width; }
...
export function renderShipCore(ctx, geometry, palette, { time = 0, lod = "high" } = {}) {
  ...
  for (const path of geometry.hullPaths) { tracePath(ctx, path); ctx.fill(); ctx.stroke(); }
  ctx.shadowBlur = 0; ctx.strokeStyle = palette.armor; for (const path of geometry.armorPaths) { tracePath(ctx, path); ctx.stroke(); }
  ctx.strokeStyle = palette.energy; ctx.globalAlpha = .65 + Math.sin(time * 2.5) * .2; for (const path of geometry.lightPaths) { tracePath(ctx, path); ctx.stroke(); }
  ctx.globalAlpha = 1; for (const [path, color] of [[geometry.cockpitPath,palette.cockpit],[geometry.reactorPath,palette.energy]]) { tracePath(ctx, path); ctx.fillStyle = color; ctx.fill(); ctx.strokeStyle = palette.edge; ctx.stroke(); }
