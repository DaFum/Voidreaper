...
function silhouettePoints(profile, radius, seed) {
  ...
  for (let index = 0; index < points; index += 1) {
    ...
    const alternating =
      profile.family === "architect" ? (index % 2 ? 0.68 : 1) : 1;
    const taper = profile.family === "lancer" && index !== 0 ? 0.72 : 1;
...
function drawMechanicalDetails(
  ctx,
  profile,
  radius,
  palette,
  ...
  const ribs = Math.max(
    2,
    Math.min(6, profile.fins || Math.ceil(profile.armor * 3)),
  );
  for (let index = 0; index < ribs; index += 1) {
    const side = index % 2 ? 1 : -1;
    const y = -radius * 0.35 + Math.floor(index / 2) * radius * 0.34;
    const x0 = side * radius * 0.2;
    const x1 = side * radius * (0.64 + profile.armor * 0.08);
    const y1 = y + side * radius * 0.08;
    ...
    ctx.beginPath();
    ctx.moveTo(x0, y - radius * 0.04);
    ctx.lineTo(x1, y1 - radius * 0.04);
    ctx.stroke();
  }
  if (profile.muzzle) {
    ctx.save();
    fillSheen(
      ctx,
      () =>
        ctx.rect(-radius * 0.16, -radius * 1.16, radius * 0.32, radius * 0.72),
      {
  ...
  if (profile.orbit) {
    ctx.save();
    if (!reducedMotion) ctx.rotate(time * 0.35);
    ctx.setLineDash([radius * 0.22, radius * 0.16]);
    ctx.strokeStyle = withAlpha(palette.energy, 0.7);
    ctx.lineWidth = Math.max(1.5, radius * 0.06);
    ctx.beginPath();
    ctx.ellipse(0, 0, radius * 1.12, radius * 0.72, 0.35, 0, TAU);
    ctx.stroke();
    ctx.restore();
  }
}
...
function drawEliteFrame(ctx, radius, color, time, reducedMotion) {
  ctx.save();
  if (!reducedMotion) ctx.rotate(-time * 0.25);
...
export function renderForgedEnemy(
  ctx,
  enemy,
  {
    frozen = false,
  ...
  if (enemy.__visualSeed === undefined)
    enemy.__visualSeed = visualHash(
      `${enemy.type}:${Math.round(enemy.x)}:${Math.round(enemy.y)}`,
    );
  ...
  const palette = mergeVisualPalette({
    armor: baseColor,
    armorLight: mixColor(baseColor, "#ffffff", 0.4),
    edge: hit ? "#ffffff" : mixColor(baseColor, "#ffffff", 0.55),
    energy:
      enemy.elite?.tint ??
      mixColor(baseColor, palette_energy_hint(baseColor), 0.5),
  ...
  const birthScale =
    enemy.birth > 0 ? Math.max(0.05, 1 - enemy.birth / 0.35) : 1;
  const { out: pts, maxD } = silhouettePoints(profile, radius, seed);

  ctx.save();
  ctx.translate(enemy.x, enemy.y);
  ctx.rotate(facing + (reducedMotion ? 0 : Math.sin(time) * 0.08));
  ctx.scale(birthScale, birthScale);
  ctx.globalAlpha *= birthScale;

  // soft grounding shadow + outer energy aura for depth
  drawContactShadow(ctx, { radius: maxD, alpha: 0.3 });
  ctx.save();
  ctx.globalCompositeOperation = "lighter";
  const aura = ctx.createRadialGradient(0, 0, maxD * 0.6, 0, 0, maxD * 1.35);
  aura.addColorStop(0, withAlpha(palette.energy, hit ? 0.5 : 0.22));
  ...
  if (hit) {
    ...
    fillSheen(ctx, () => tracePolygon(ctx, pts), {
      light: mixColor(shade(baseColor), "#ffffff", 0.45),
      base: shade(baseColor),
      dark: mixColor(shade(baseColor), "#000000", 0.5),
      span: maxD,
    });
  }
  // outline
  ...
  tracePolygon(ctx, pts);
  ctx.clip();
  tracePolygon(ctx, pts, 0.62);
  ctx.fillStyle = withAlpha("#000000", 0.28);
  ctx.fill();
  // spokes from center to each vertex
  ctx.strokeStyle = withAlpha(palette.armorLight ?? palette.armor, 0.35);
  ctx.lineWidth = 1;
  pts.forEach((p) => {
    ctx.beginPath();
    ctx.moveTo(p.x * 0.58, p.y * 0.58);
  ...
  if (hasRim) {
    ctx.strokeStyle = withAlpha(palette.rim ?? "#ffffff", 0.55);
    ctx.lineWidth = 1.2;
    ctx.stroke();
  }
  ctx.restore();
  ...
  pts.forEach((p) => {
    ctx.beginPath();
    ctx.arc(p.x * 0.88, p.y * 0.88, Math.max(1, radius * 0.05), 0, TAU);
    ctx.fill();
  });
  ctx.restore();
  ...
  drawVoidCore(ctx, {
    radius: radius * (enemy.boss ? 0.3 : 0.26),
    palette,
    time,
    seed,
    reducedMotion,
    intensity: hit ? 1 : 0.92,
  });
  ...
  if (enemy.elite)
    drawEliteFrame(ctx, radius, enemy.elite.tint, time, reducedMotion);
  if (enemy.boss) {
    ctx.save();
    if (!reducedMotion) ctx.rotate(time * 0.22);
    ctx.strokeStyle = "#ffd166";
    ctx.lineWidth = 2;
    for (let ring = 0; ring < 2; ring += 1) {
      ctx.setLineDash([8 + ring * 3, 5]);
      ctx.beginPath();
      ctx.arc(0, 0, radius * (1.08 + ring * 0.2), ring * 0.5, TAU - ring * 0.4);
      ctx.stroke();
    }
    ctx.restore();
  }
  ...
  if (enemy.shielded && enemy.birth <= 0 && target) {
    ...
    ctx.strokeStyle = withAlpha(palette.energy, 0.28);
    ctx.lineWidth = 9;
    ctx.beginPath();
    ctx.arc(
      enemy.x,
  ...
  if (
    ...
    grad.addColorStop(0, mixColor(c, "#ffffff", 0.3));
