export function renderEntities(
  ...
  const height =
    viewport.height ?? context.canvas.clientHeight ?? context.canvas.height;
  ...
  for (const enemy of run.enemies) {
    ...
    if (
      ...
      enemy.x - r > cullRight ||
      enemy.y + r < cullTop ||
      enemy.y - r > cullBottom
    ) {
      continue;
    }
    context.fillStyle = enemy.color ?? "#ff2d78";
    context.beginPath();
    context.arc(enemy.x, enemy.y, r, 0, Math.PI * 2);
    context.fill();
  }
  ...
  for (const projectile of run.projectiles) {
    ...
    // Spatial culling: skip rendering if entirely outside viewport AABB
    if (
      projectile.x + r < cullLeft ||
      projectile.x - r > cullRight ||
      projectile.y + r < cullTop ||
      projectile.y - r > cullBottom
    ) {
      continue;
    }
    context.fillStyle = projectile.color ?? "#4cc9f0";
    context.beginPath();
    context.arc(projectile.x, projectile.y, r, 0, Math.PI * 2);
    context.fill();
  }
  const rendered = assemblyRenderer?.renderPlayerShip(context, {
    geometrySnapshot: assemblyGeometry?.getSnapshot(),
    position: run.player,
  ...
  if (!rendered) {
    ...
    context.beginPath();
    context.arc(run.player.x, run.player.y, run.player.radius, 0, Math.PI * 2);
    context.fill();
  }
  context.restore();
}
