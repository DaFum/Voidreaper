...
function drawGrid(ctx, profile, bounds, time, reducedMotion) {
  ...
  if (profile.grid === "cathedral") {
    ...
    ctx.save();
    ctx.rotate(reducedMotion ? 0 : time * 0.025);
    ctx.beginPath();
    ...
    const maxDist = Math.sqrt(
      Math.max(Math.abs(bounds.minX), Math.abs(bounds.maxX)) ** 2 +
    ...
    for (let x = startX; x <= bounds.maxX; x += TILE) {
      ...
      ctx.lineTo(
        profile.grid === "fracture" ? x + TILE * 0.45 : x,
        bounds.maxY,
      );
    }
    for (let y = startY; y <= bounds.maxY; y += TILE) {
      ctx.moveTo(bounds.minX, y);
      ctx.lineTo(bounds.maxX, profile.grid === "salvage" ? y + TILE * 0.18 : y);
    }
    ctx.stroke();
  }
...
function motifSprite(profile, seed) {
  ...
  spriteCtx.setTransform(
    ...
    0,
    MOTIF_BAKE_SCALE,
    extent * MOTIF_BAKE_SCALE,
    extent * MOTIF_BAKE_SCALE,
  );
  drawMotifShape(spriteCtx, profile, seed, 0, true);
  const sprite = { canvas, extent };
  if (motifSprites.size >= MOTIF_CACHE_LIMIT)
    motifSprites.delete(motifSprites.keys().next().value);
...
function drawMotif(ctx, profile, x, y, seed, time, reducedMotion) {
  ...
  ctx.save();
  ctx.translate(x, y);
  drawMotifShape(ctx, profile, seed, time, reducedMotion);
  ctx.restore();
}

function drawMotifShape(ctx, profile, seed, time, reducedMotion) {
  const localPalette = mergeVisualPalette({
    armor: profile.palette.grid,
    edge: profile.palette.accent,
    energy: profile.palette.accent,
    void: profile.palette.void,
  });
  const size = 18 + seededUnit(seed, 1) * 28;
  ctx.rotate(seededUnit(seed, 2) * Math.PI * 2);
  ctx.globalAlpha = 0.12 + seededUnit(seed, 3) * 0.13;
  if (profile.id === "shattered-approach") {
    ctx.fillStyle = profile.palette.grid;
    ctx.strokeStyle = profile.palette.accent;
    ctx.beginPath();
    ctx.moveTo(0, -size);
    ...
    traceChamferedPlate(ctx, { width: size * 1.8, height: size, chamfer: 7 });
    ctx.fillStyle = profile.palette.floor;
    ctx.strokeStyle = profile.palette.accent;
    ctx.fill();
    ctx.stroke();
    drawCracks(ctx, {
    ...
    ctx.quadraticCurveTo(0, size, size * 0.7, size * 0.3);
    ctx.stroke();
    ...
    ctx.setLineDash([6, 5]);
    ctx.beginPath();
    ctx.arc(0, 0, size, 0, Math.PI * 1.72);
    ctx.stroke();
    ctx.setLineDash([]);
...
export function renderRegionWorld(
  ctx,
  {
  ...
  const bounds = {
    minX: Math.max(-arena, camera.x - width * 0.72),
    maxX: Math.min(arena, camera.x + width * 0.72),
    minY: Math.max(-arena, camera.y - height * 0.72),
    maxY: Math.min(arena, camera.y + height * 0.72),
  };
  ...
  gradient.addColorStop(0, profile.palette.floor);
  gradient.addColorStop(1, "#020307");
  ctx.globalAlpha = baseAlpha * floorAlpha;
  ctx.fillStyle = gradient;
  ctx.fillRect(-arena, -arena, arena * 2, arena * 2);
  ctx.globalAlpha = baseAlpha;
  ctx.save();
  ctx.beginPath();
  ctx.rect(-arena, -arena, arena * 2, arena * 2);
  ctx.clip();
  if (!lowDetail)
    renderRegionParallaxBackdrop(ctx, {
      regionId,
      camera,
  ...
  const startX = Math.floor(bounds.minX / TILE) * TILE,
    startY = Math.floor(bounds.minY / TILE) * TILE;
  for (let x = startX; x <= bounds.maxX; x += TILE)
    for (let y = startY; y <= bounds.maxY; y += TILE) {
