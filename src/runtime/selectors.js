export function selectStat(run, statId, fallback = 0) {
  const calculated = run.services?.stats?.calculate(statId, {
    run,
    player: run.player,
  });
...
export function selectStatBreakdown(run, statId) {
  return (
    run.services?.stats?.calculate(statId, {
      run,
      player: run.player,
...
export const selectHullRatio = (run) =>
  run.player.maxHull > 0 ? run.player.hull / run.player.maxHull : 0;
export const selectEnergyRatio = (run) =>
  run.player.resources.maxEnergy > 0
...
export const selectActiveTags = (run) =>
  [...(run.build.tags.totals?.entries() ?? run.build.tags.entries())].filter(
    ([, value]) => value > 0,
  );
