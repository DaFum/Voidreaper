...
export async function bootstrap() {
  ...
  if (import.meta.env.DEV) { const firstModule=()=>Object.values(services.currentAssembly?.getSnapshot().nodesById??{}).find(node=>node.moduleInstanceId);const scenarios={damageSingleModule:(amount=20)=>{const node=firstModule();return node?services.moduleDamage.applyDamage(node.nodeId,amount,"debug"):null;},detachParentWithBridge:()=>({instruction:"Add secondary connection, then apply lethal damage to its parent.",snapshot:services.currentAssembly?.getSnapshot()}),detachParentWithoutBridge:()=>{const node=firstModule();return node?services.moduleDamage.applyDamage(node.nodeId,9999,"debug"):null;},areaDamageCluster:(amount=30)=>Object.values(services.currentAssembly?.getSnapshot().nodesById??{}).filter(node=>node.moduleInstanceId).slice(0,3).map(node=>services.moduleDamage.applyDamage(node.nodeId,amount/3,"area-debug")),directCoreAttack:(amount=10)=>{const run=controller.run;if(run)run.player.hull=Math.max(0,run.player.hull-amount);return run?.player.hull;},repairAndRemount:()=>({detached:services.currentAssembly?.getSnapshot().detachedItems??[],resources:controller.run?.resources}),massExtremes:()=>services.flightProfile?.getProfile(),recoilAsymmetry:()=>controller.run?services.recoil?.apply(controller.run.player,{weaponClass:"rail",worldPosition:{x:80,y:0},direction:{x:0,y:-1}}):null};globalThis.__VOIDREAPER_DEBUG__ = { ...(globalThis.__VOIDREAPER_DEBUG__ ?? {}), assembly: { getSnapshot: () => services.currentAssembly?.getSnapshot() ?? null, getGeometry: () => services.assemblyGeometry?.getSnapshot() ?? null, getHitZones:()=>services.hitZones?.all()??[],getFlightProfile:()=>services.flightProfile?.getProfile()??null,getPending: () => services.pendingMounts?.values() ?? [], getQuickMount: () => services.quickMount?.session ?? null, getWorkbench: () => services.assemblyWorkbench?.model() ?? null, scenarios, assemblyVisualGallery: () => ({ cores: getCoreGeometryIds(), modules: MODULE_VISUAL_PROFILES.map(profile => profile.rendererId), damageStates: ["intact","armor-broken","core-disrupted"] }) } }; }
