...
export function renderSettingsScreen(root, settings, onChange) {
  ...
  root.onchange = event => {
    ...
    if (setting) settings[setting] = event.target.type === "checkbox" ? event.target.checked : Number(event.target.value);
    document.documentElement.style.setProperty("--ui-scale", settings.uiScale);
    document.documentElement.dataset.reducedMotion = String(settings.reducedMotion);
    document.documentElement.dataset.colorPatterns = String(settings.colorPatterns);
    onChange(settings);
  };
