...
const TOGGLES = [
  ["reducedMotion", "Reduced Motion", "settings-reduced-motion"],
  ["screenShake", "Screen Shake"],
  ["damageFlashes", "Damage Flashes"],
  ["crt", "CRT Overlay"],
  ["largeTouchControls", "Große Touch-Buttons", "touch-controls"],
  ["colorPatterns", "Zustandsmuster", "settings-color-patterns"],
];
  ...
  root.onkeydown = (event) => {
    ...
    const displacedAction =
      settings.bindings[event.code] !== binding
        ? settings.bindings[event.code]
        : null;
    for (const code in settings.bindings) {
      const action = settings.bindings[code];
      if (action === binding || code === event.code)
        delete settings.bindings[code];
    }
    settings.bindings[event.code] = binding;
    ...
    if (displacedAction) {
      ...
      const displacedInput = root.querySelector(
        `[data-binding="${displacedAction}"]`,
      );
