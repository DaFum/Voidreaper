import {
  CODEX_CATEGORIES,
  DISCOVERY_LEVELS,
} from "../../content/codex/codex-categories.js";
import { escapeHtml } from "../escape-html.js";
import { buildFocusSelector } from "../focus-restore.js";

export function renderCodexScreen(
  root,
  { entries, filters = {}, onFilter = () => {} },
) {
  if (!root) return;
  const activeElement =
  ...
  const grid = entries.length
    ? entries
        .map(
          (entry) =>
            `<article data-level="${escapeHtml(entry.level)}"><span>${escapeHtml(entry.category)} · ${escapeHtml(entry.level)}</span><h3>${escapeHtml(entry.name)}</h3><p>${escapeHtml(entry.description)}</p><small>${entry.tags.map(escapeHtml).join(" · ")}</small></article>`,
        )
        .join("")
    : `<div class="codex__empty">KEINE PASSENDEN SIGNALE</div>`;
  ...
  for (const control of root.querySelectorAll("select,input"))
    control.addEventListener("change", () =>
      onFilter({
        ...
        tag: root.querySelector("[data-tag]").value,
        source: root.querySelector("[data-source]").value,
      }),
    );
}

export function renderBuildHistory(root, builds, onToggleFavorite = () => {}) {
  root.innerHTML = `<div class="codex__grid">${builds.map((build) => `<article><span>${escapeHtml(build.result)} · SEED ${build.seed}</span><h3>${escapeHtml(build.ship ?? "Unknown")} / ${escapeHtml(build.weapon ?? "Unknown")}</h3><p>${build.modules.map(escapeHtml).join(" · ")}</p><button type="button" data-favorite="${escapeHtml(build.id)}" aria-label="${build.favorite ? "Aus Favoriten entfernen" : "Als Favorit markieren"}" title="${build.favorite ? "Aus Favoriten entfernen" : "Als Favorit markieren"}" aria-pressed="${build.favorite ? "true" : "false"}"><span>${build.favorite ? "★" : "☆"}</span> <small aria-hidden="true">${build.favorite ? "Favorit" : "Favorisieren"}</small></button></article>`).join("")}</div>`;
  root.onclick = (event) => {
    const button = event.target.closest("[data-favorite]");
    if (button) onToggleFavorite(button.dataset.favorite);
  };
}
