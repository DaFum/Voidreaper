...
export function renderChallengesScreen(root, challenges, progress = {}) {
  root.innerHTML = `<section class="codex"><header>CHALLENGE UPLINK <b>${escapeHtml(Object.values(progress).filter((value) => value.claimed).length)}/${escapeHtml(challenges.length)}</b></header><div class="codex__grid">${challenges
    .map(
      (challenge) =>
