...
export function renderAnomalyScreen(root, { event, onChoose }) {
  if (!root) return;
  root.innerHTML = `<section class="service-screen anomaly"><header>UNKNOWN SIGNATURE <b>RISK EVENT</b></header><h2>${escapeHtml(event.name)}</h2><p>${escapeHtml(event.description)}</p><div class="anomaly__choices"></div></section>`;
