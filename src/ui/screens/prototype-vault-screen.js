...
export function renderPrototypeVault(
  root,
  {
    items,
    capacity,
  ...
  root.innerHTML = `<section class="codex"><header>PROTOTYPE VAULT <b>${escapeHtml(items.length)}/${escapeHtml(capacity)} · OVERFLOW ${escapeHtml(overflowCount)}</b></header><div class="codex__filters"><label>Familie <input data-family value="${filterValue("family")}" placeholder="Familie"></label><label>Tag <input data-tag value="${filterValue("tag")}" placeholder="Tag"></label><label>Seltenheit <input data-rarity value="${filterValue("rarity")}" placeholder="Seltenheit"></label><label>Herkunft <input data-source value="${filterValue("source")}" placeholder="Herkunft"></label></div><div class="codex__grid">${items.length ? items.map((item) => `<article><span>${escapeHtml(item.rarity)} · ${escapeHtml(describeStability(item))}</span><h3>${escapeHtml(item.name)}</h3><p>${(item.tags ?? []).map((tag) => escapeHtml(tag.id ?? tag)).join(" · ")}</p><button type="button" data-favorite="${escapeHtml(item.instanceId)}" aria-label="${item.favorite ? "Aus Favoriten entfernen" : "Als Favorit markieren"}" title="${item.favorite ? "Aus Favoriten entfernen" : "Als Favorit markieren"}" aria-pressed="${item.favorite ? "true" : "false"}"><span>${item.favorite ? "★" : "☆"}</span> <small aria-hidden="true">${item.favorite ? "Favorit" : "Als Favorit"}</small></button><button type="button" data-dismantle="${escapeHtml(item.instanceId)}" ${item.favorite ? 'disabled title="Zerlegen – Favoriten können nicht zerlegt werden" aria-label="Zerlegen – Favoriten können nicht zerlegt werden"' : ""}>Zerlegen <small aria-hidden="true">${item.favorite ? "(Gesperrt)" : ""}</small></button></article>`).join("") : `<div class="vault-empty"><strong>${hasFilter ? "KEINE TREFFER" : "VAULT LEER"}</strong><span>${hasFilter ? "Kein Prototyp entspricht den aktuellen Filtern." : "Extrahiere Prototypen aus Runs, um sie hier zu sichern."}</span></div>`}</div></section>`;
  if (focusedSelector) {
    const newTarget = root.querySelector(focusedSelector);
    if (newTarget) {
      newTarget.focus();
      if (newTarget.tagName === "INPUT" && selectionStart != null)
        newTarget.setSelectionRange(selectionStart, selectionEnd);
    }
  }
  root.onclick = (event) => {
    const favorite = event.target.closest("[data-favorite]");
    const dismantle = event.target.closest("[data-dismantle]");
    if (favorite) onFavorite(favorite.dataset.favorite);
    if (dismantle) onDismantle(dismantle.dataset.dismantle);
  };
  if (onFilter)
    root.onchange = (event) => {
      if (!event.target.closest(".codex__filters")) return;
      onFilter({
        family: root.querySelector("[data-family]").value.trim(),
        tag: root.querySelector("[data-tag]").value.trim(),
        rarity: root.querySelector("[data-rarity]").value.trim(),
        source: root.querySelector("[data-source]").value.trim(),
      });
    };
}
