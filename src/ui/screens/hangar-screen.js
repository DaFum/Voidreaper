...
export function createHangarScreen(
  ...
  const renderCatalog = (content, definitions) => {
    ...
    const refreshCatalog = () => {
      ...
      if (selectedEntry.state === "locked") {
        html = `<header><div><small>GESPERRT</small><strong>${escapeHtml(selectedEntry.definition.name)}</strong></div><button type="button" data-catalog-selection-close aria-label="Auswahl schließen" title="Auswahl schließen"><span aria-hidden="true">×</span></button></header><p>${escapeHtml(selectedEntry.unlockLabel)}</p><p data-catalog-message role="status" aria-live="polite"></p>`;
      } else {
        ...
        html = `<header><div><small>${escapeHtml(selectedEntry.state === "equipped" ? "AUSGERÜSTET" : "VERFÜGBAR")}</small><strong>${escapeHtml(selectedEntry.definition.name)}</strong></div><button type="button" data-catalog-selection-close aria-label="Auswahl schließen" title="Auswahl schließen"><span aria-hidden="true">×</span></button></header><div class="catalog-selection__slots">${slotActions}</div><p data-catalog-message role="status" aria-live="polite"></p>`;
      }
  ...
  const render = () => {
    ...
    container.innerHTML = `<div class="hangar-navigation"><div class="hangar-tabs-shell" data-overflow-start="false" data-overflow-end="false"><button type="button" class="hangar-tabs-scroll" data-hangar-scroll="previous" aria-label="Vorherige Bereiche anzeigen" title="Vorherige Bereiche anzeigen"><span aria-hidden="true">‹</span></button><nav class="hangar-tabs" role="tablist" aria-label="Hangar-Bereiche">${TABS.map((name) => `<button type="button" id="hangar-panel-tab-${tutorialId(name)}" data-hangar-tab="${escapeHtml(name)}" data-tutorial-id="${tutorialId(name)}" role="tab" aria-selected="${name === tab}" tabindex="${name === tab ? "0" : "-1"}">${escapeHtml(name)}</button>`).join("")}</nav><button type="button" class="hangar-tabs-scroll" data-hangar-scroll="next" aria-label="Weitere Bereiche anzeigen" title="Weitere Bereiche anzeigen"><span aria-hidden="true">›</span></button></div><button type="button" class="hangar-area-toggle" data-hangar-area-toggle aria-expanded="false"><span>AKTIVER BEREICH</span><strong>${escapeHtml(tab)}</strong><i aria-hidden="true">⌄</i></button><section class="hangar-area-panel" data-hangar-area-panel role="dialog" aria-label="Hangar-Bereich wählen" hidden><header><span>BEREICH WÄHLEN</span><button type="button" data-hangar-area-close aria-label="Bereichsauswahl schließen" title="Bereichsauswahl schließen"><span aria-hidden="true">×</span></button></header><div>${TABS.map((name) => `<button type="button" data-hangar-area="${escapeHtml(name)}"${name === tab ? ' aria-current="page"' : ""}>${escapeHtml(name)}</button>`).join("")}</div></section></div><section class="hangar-stage" role="tabpanel" aria-labelledby="${activeTabId}" data-active-tab="${escapeHtml(tab)}"><header class="hangar-signal"><span>VR // HANGAR LINK · ◇${currentCurrencies.voidShards ?? 0} · ⬡${currentCurrencies.bossCores ?? 0} · ◉${currentCurrencies.anomalyData ?? 0} · ✦${currentCurrencies.challengeSeals ?? 0} · ▱${currentCurrencies.salvageFragments ?? 0}</span><b>${ships.length} FRAMES · ${weapons.length} WEAPONS · ${reactors.length} CORES · ${modules.length} MODULES</b></header><div class="hangar-content"></div></section>`;
