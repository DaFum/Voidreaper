...
export function renderLoadoutScreen(container, inspection, loadout,{blueprints=[],activeBlueprintId=null,choicesBySlot={},onBlueprintChange,onEquip,onUnequip,onNavigate}={}) {
  ...
  const openPicker = (slot, index) => {
    ...
    picker.hidden = false;
    picker.innerHTML = `<header><small>${escapeHtml(slot)} ${escapeHtml(index + 1)}</small><button type="button" data-picker-close aria-label="Auswahl schließen">×</button></header><div class="item-catalog">${choices.map(definition => `<button type="button" class="item-card" data-choice="${escapeHtml(definition.id)}" aria-pressed="${definition.id === current?.definitionId}"><strong>${escapeHtml(definition.name ?? definition.id)}</strong><small>${escapeHtml(definition.description ?? definition.id)}</small></button>`).join("") || emptyState}</div><button type="button" data-unequip${current ? "" : " disabled"}>ENTFERNEN</button>`;
