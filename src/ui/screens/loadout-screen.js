...
export function loadoutStatus(inspection) {
  ...
  return {
    percent: Math.round(inspection.load.ratio * 100),
    tier: inspection.load.tier,
  };
}
...
export function loadoutDefinitionNames(inspection) {
  return new Map(
    (inspection?.sources ?? [])
      .filter(
        (source) => source && (source.id ?? source.definitionId) && source.name,
      )
...
export function renderLoadoutScreen(
  ...
  const frameName =
    inspection?.sources?.find((source) => source.slot === "ship")?.name ??
    loadout.slots.ship?.[0]?.definitionId ??
    "NO SHIP";
  const slots = Object.entries(LOADOUT_SLOT_LAYOUT)
    .flatMap(([slot, count]) =>
      Array.from({ length: count }, (_, index) => {
        const item = loadout.slots[slot]?.[index] ?? null;
        const itemName = item
          ? (definitionNames.get(item.definitionId) ?? item.definitionId)
          : "EMPTY";
        const classes =
          slot === "ship" ? "ship-core loadout-slot" : "loadout-slot";
        return `<button type="button" class="${classes}" data-slot="${escapeHtml(slot)}" data-index="${index}" aria-label="${escapeHtml(slot)} ${escapeHtml(index + 1)}: ${escapeHtml(slot === "ship" ? frameName : itemName)}" title="${escapeHtml(slot)} ${escapeHtml(index + 1)}: ${escapeHtml(slot === "ship" ? frameName : itemName)}"><span>${slot === "ship" ? "FRAME" : escapeHtml(slot)}</span><strong>${escapeHtml(slot === "ship" ? frameName : itemName)}</strong>${slot === "ship" ? `<small>${escapeHtml(status.percent)}% ${escapeHtml(status.tier)}</small>` : ""}</button>`;
      }),
    )
    .join("");
  ...
  const openPicker = (slot, index) => {
    ...
    const emptyState = `<div class="loadout-picker__empty" data-picker-empty><p>Keine freigeschalteten Komponenten. Neue Optionen erhältst du über Forschung oder Bergung.</p><div><button type="button" data-picker-navigate="Forschung">Forschung öffnen</button><button type="button" data-picker-navigate="Bergung">Bergung öffnen</button></div></div>`;
    picker.hidden = false;
    picker.innerHTML = `<header><small>${escapeHtml(slot)} ${escapeHtml(index + 1)}</small><button type="button" data-picker-close aria-label="Auswahl schließen" title="Auswahl schließen"><span aria-hidden="true">×</span></button></header><div class="item-catalog">${choices.map((definition) => `<button type="button" class="item-card" data-choice="${escapeHtml(definition.id)}" aria-pressed="${definition.id === current?.definitionId}"><strong>${escapeHtml(definition.name ?? definition.id)}</strong><small>${escapeHtml(definition.description ?? definition.id)}</small></button>`).join("") || emptyState}</div><button type="button" data-unequip${current ? "" : ' disabled aria-label="Entfernen nicht möglich (Slot ist bereits leer)" title="Slot ist bereits leer"'}>ENTFERNEN${current ? "" : ' <small aria-hidden="true">(Leer)</small>'}</button>`;
    picker.onclick = (event) => {
      const definitionId =
        event.target.closest("[data-choice]")?.dataset.choice;
      if (definitionId) {
        onEquip?.(slot, index, definitionId);
        closePicker();
        return;
      }
      const area = event.target.closest("[data-picker-navigate]")?.dataset
        .pickerNavigate;
      if (area) {
        onNavigate?.(area);
        return;
      }
      if (event.target.closest("[data-unequip]")) {
  ...
  container
    .querySelector(".loadout-orbit")
    ?.addEventListener("click", (event) => {
      const control = event.target.closest("[data-slot]");
      if (control)
        openPicker(control.dataset.slot, Number(control.dataset.index));
    });
  container
    .querySelector("[data-blueprint]")
    ?.addEventListener("change", (event) =>
      onBlueprintChange?.(event.target.value || null),
    );
}
