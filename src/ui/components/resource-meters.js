...
export function updateResourceMeters(
  root,
  { energy, heat, corruption, load, scrap = 0, flux = 0 },
) {
  setMeter(
    root,
    "energy",
    energy.maximum ? energy.value / energy.maximum : 0,
    `${Math.ceil(energy.value)} / ${energy.maximum}`,
  );
  setMeter(root, "heat", heat / 100, `${Math.round(heat)}°`);
  setMeter(
    root,
    "corruption",
    Math.min(corruption, 100) / 100,
  ...
  setMeter(
    root,
    "load",
    Math.min(loadRatio, 1.6) / 1.6,
    `${Math.round(loadRatio * 100)}% ${loadTier.toUpperCase()}`,
  );
