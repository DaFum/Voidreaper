export function placeTutorialCard(target, card, viewport, gap = 16) {
  const margin = 8;
  ...
  if (
    target.width >= viewport.width * 0.9 &&
    target.height >= viewport.height * 0.9
  )
    return {
  ...
  const spaceBelow = viewport.height - target.bottom - gap,
    spaceAbove = target.top - gap,
    side =
      spaceBelow >= card.height || spaceBelow >= spaceAbove ? "bottom" : "top";
  let top =
    side === "bottom" ? target.bottom + gap : target.top - gap - card.height;
