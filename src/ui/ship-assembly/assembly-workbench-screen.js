...
export function createAssemblyWorkbenchScreen(root, { onAction } = {}) {
  ...
  root.addEventListener(
    ...
    (event) => {
      ...
      if (action && !control.disabled) onAction?.(action, control.dataset);
    },
    { signal: abort.signal },
  );
  return {
    ...
    renderPorts(ports, { selectedPortId = null, selectedNodeId = null } = {}) {
      root.querySelector('[data-role="ports"]').innerHTML = ports
        .map((port) => {
    ...
    setInspector(content) {
      root.querySelector('[data-role="inspector"]').replaceChildren(content);
    },
  };
}
