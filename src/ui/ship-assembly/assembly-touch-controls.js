export function attachAssemblyTouchControls(
  element,
  { onPrevious, onNext, onSelect, onDetails } = {},
) {
  let start = null,
    longPress = null;
  const clear = () => {
  ...
  const down = (event) => {
    ...
    longPress = setTimeout(() => onDetails?.(), 520);
  };
  const move = (event) => {
    if (!start) return;
    const point = event.touches?.[0] ?? event;
    if (
      (point.clientX - start.x) * (point.clientX - start.x) +
        (point.clientY - start.y) * (point.clientY - start.y) >
      100
    )
      clear();
  };
  const up = (event) => {
    ...
    const point = event.changedTouches?.[0] ?? event,
      ...
      duration = performance.now() - start.time;
    clear();
    if (Math.abs(dx) > 48 && Math.abs(dx) > Math.abs(dy))
      dx < 0 ? onNext?.() : onPrevious?.();
    else if (duration < 500) onSelect?.(event);
    start = null;
  };
  const cancel = () => {
    clear();
