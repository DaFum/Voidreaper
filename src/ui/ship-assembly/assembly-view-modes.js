...
export const ASSEMBLY_VIEW_MODE_LABELS = Object.freeze({
  normal: "KONSTRUKTION",
  structure: "STRUKTUR",
  energy: "ENERGIE",
  damage: "SCHADEN",
...
export function renderAssemblyToolbar(root, active, onChange) {
  root.innerHTML = Object.values(ASSEMBLY_VIEW_MODES)
    .map(
      (mode) =>
        `<button type="button" data-mode="${mode}" aria-pressed="${mode === active}">${escapeHtml(ASSEMBLY_VIEW_MODE_LABELS[mode])}</button>`,
    )
    .join("");
  root.onclick = (event) => {
...
export function getViewModeOverlay(
  mode,
  { assembly, geometry, flightProfile } = {},
) {
  if (mode === ASSEMBLY_VIEW_MODES.STRUCTURE)
    return {
      connections: geometry.connections,
      labels: geometry.nodes.map((node) => ({
        position: node.worldPosition,
        text: node.isRoot ? "KERN" : `T${branchDepthFor(node, assembly)}`,
      })),
    };
  if (mode === ASSEMBLY_VIEW_MODES.ENERGY)
    return {
      connections: geometry.connections.map((connection) => ({
        ...connection,
        label: connection.cable.energyClass,
      })),
      labels: [],
    };
  if (mode === ASSEMBLY_VIEW_MODES.DAMAGE)
    return {
      labels: geometry.nodes.map((node) => ({
        position: node.worldPosition,
        text: node.isRoot
          ? "KERN"
          : `${Math.round(node.armorIntegrity ?? 0)}A / ${Math.round(node.coreIntegrity ?? 0)}C`,
      })),
    };
  if (mode === ASSEMBLY_VIEW_MODES.FLIGHT)
    return {
      centerOfMass: flightProfile?.centerOfMass,
      thrustVectors: flightProfile?.thrustVectors ?? [],
      labels: [
        {
          position: { x: 0, y: 0 },
          text: `MASSE ${flightProfile?.totalMass ?? "—"}`,
        },
      ],
    };
  return {};
...
export function renderViewModeOverlay(ctx, overlay = {}) {
  ...
  for (const label of layoutOverlayLabels([
    ...connectionLabels,
    ...(overlay.labels ?? []),
  ]))
    ctx.fillText(label.text, label.position.x + 6, label.position.y - 6);
  if (overlay.centerOfMass) {
    const { x, y } = overlay.centerOfMass;
    ctx.beginPath();
    ctx.moveTo(x - 10, y);
    ctx.lineTo(x + 10, y);
    ctx.moveTo(x, y - 10);
  ...
  for (const vector of overlay.thrustVectors ?? []) {
    const end = {
      x: vector.position.x + vector.direction.x * 28 * vector.strength,
      y: vector.position.y + vector.direction.y * 28 * vector.strength,
    };
    ctx.beginPath();
