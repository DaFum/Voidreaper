...
test("createInputController initializes with bindings and touch stick", () => {
  ...
  assert.deepEqual(
    controller.bindings,
    DEFAULT_BINDINGS,
    "Initializes with default bindings",
  );
...
test("createInputController rebinds actions and triggers custom ones", () => {
  ...
  assert.equal(
    controller.bindings["KeyZ"],
    "move-up",
    "Action rebounded successfully to KeyZ",
  );
  assert.equal(
    controller.bindings["KeyW"],
    undefined,
    "Old duplicate binding for KeyW is cleared",
  );
