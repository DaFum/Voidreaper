...
import {
  TUTORIAL_EVENTS,
  isTutorialEvent,
} from "../../../src/features/tutorial/tutorial-events.js";
import {
  createTutorialService,
  shouldAutoOfferFoundations,
  shouldExitFoundationsRun,
  tutorialAvailableSystems,
} from "../../../src/features/tutorial/tutorial-service.js";

test("Bastion is regular research progress, not tutorial progress", () => {
  assert.deepEqual(
    SHIPS.filter((ship) => ship.unlockSource === "onboarding"),
    [],
  );
  assert.equal(
    RESEARCH_TREE.find(
      (node) => node.id === "engineering-load",
    ).unlocks.includes("bastion"),
    true,
  );
});
...
function fixture(initial = {}) {
  ...
  const chapters = [
    {
      ...
      steps: [
        { id: "intro", kind: "explanation", title: "Start", body: "Los" },
        {
          id: "move",
          kind: "action",
          event: TUTORIAL_EVENTS.MOVEMENT_USED,
          matches: (payload) => payload.magnitude > 0.25,
        },
  ...
  return {
    state,
    eventBus,
    service: createTutorialService({ saveStore, eventBus, chapters }),
  };
}
...
test("coordinator advances only from the matching successful event", async () => {
  ...
  eventBus.emit(TUTORIAL_EVENTS.MOVEMENT_USED, { magnitude: 0.7 });
...
test("duplicate events cannot advance a later tutorial step", async () => {
  ...
  await service.advanceExplanation();
  eventBus.emit(TUTORIAL_EVENTS.MOVEMENT_USED, { magnitude: 0.7 });
  eventBus.emit(TUTORIAL_EVENTS.MOVEMENT_USED, { magnitude: 0.8 });
  await service.whenIdle();
  assert.equal(service.snapshot().active.stepId, "dodge");
  service.destroy();
});
...
test("finishing the last step announces the chapter completion exactly once", async () => {
  const { service, eventBus } = fixture();
  const completed = [];
  eventBus.on(TUTORIAL_EVENTS.CHAPTER_COMPLETED, (payload) =>
    completed.push(payload.chapterId),
  );
  await service.start("foundations");
  await service.advanceExplanation();
  eventBus.emit(TUTORIAL_EVENTS.MOVEMENT_USED, { magnitude: 0.7 });
...
test("availability ids keep chapters staggered until their system is discovered", () => {
    ...
    chapters: [
      {
        id: "always",
        title: "Always",
        steps: [{ id: "intro", kind: "explanation" }],
      },
      {
        id: "later",
        title: "Later",
        availabilityId: "campaign-map",
        steps: [{ id: "intro", kind: "explanation" }],
      },
    ],
  });
  service.hydrate(state.tutorial);
  assert.deepEqual(
    service.available().map((chapter) => chapter.available),
    [true, false],
  );
  assert.deepEqual(
    service
      .available({ availableSystems: ["campaign-map"] })
      .map((chapter) => chapter.available),
    [true, true],
  );
  state.tutorial.completedChapters.later = true;
  service.hydrate(state.tutorial);
  assert.equal(
    service.available().find((chapter) => chapter.id === "later").available,
    true,
  );
  service.destroy();
});
...
test("research stays locked until the profile can perform its interactive step", () => {
  ...
  assert.deepEqual(tutorialAvailableSystems(fresh, { canResearch: true }), [
    "loadout",
    "research",
  ]);
  assert.deepEqual(
    tutorialAvailableSystems(fresh, { campaignMapDiscovered: true }),
    ["loadout", "campaign-map"],
  );
});
...
test("legacy skipped foundations are not offered again after reload", () => {
  assert.equal(
    shouldAutoOfferFoundations({
      ...
      active: null,
      completedChapters: {},
      skippedChapters: { foundations: true },
    }),
    false,
  );
  assert.equal(
...
test("foundations exits its isolated run after completion, skip, or stop", () => {
  ...
  assert.equal(
    shouldExitFoundationsRun(foundations, {
      active: { chapterId: "run-navigation" },
    }),
    true,
  );
  assert.equal(
