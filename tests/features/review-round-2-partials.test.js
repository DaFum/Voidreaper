...
test("M25: declared latent effect executes as a silent no-op", () => {
  ...
  try {
    ...
    assert.equal(
      registry.execute({ id: "ship-bastion-entrensh-typo" }, {}),
      null,
    );
...
test("L14: describeStability derives labels from numeric stability and corruption", () => {
  ...
  assert.equal(
    describeStability({ stability: 100, corruptionLevel: 80 }),
    "corrupted",
  );
  ...
  assert.equal(
    describeStability({ stability: "stable", corruptionLevel: 80 }),
    "corrupted",
  );
  assert.equal(
    describeStability({ stability: 100, corruption: 80 }),
    "corrupted",
  );
});
...
test("L14: salvage outcomes write numeric stability and the vault filter matches labels", async () => {
  const save = {
    wreckSignals: {
      "sig-1": {
        id: "sig-1",
        status: "pending",
        itemSnapshot: { instanceId: "item-1", seed: 3, affixes: [] },
      },
    },
    inventory: {},
    blueprints: {},
  ...
  const service = createSalvageMissionService({
    update: async (apply) => apply(save),
  });
  const mission = { signalId: "sig-1" };
  await service.apply(mission, "damaged");
  assert.equal(save.inventory["item-1"].stability, 45);

  save.wreckSignals["sig-2"] = {
    id: "sig-2",
    status: "pending",
    itemSnapshot: { instanceId: "item-2", seed: 4, affixes: [] },
  };
  ...
  assert.deepEqual(
    vault.filter({ stability: "damaged" }).map((item) => item.instanceId),
    ["item-1"],
  );
  assert.deepEqual(
    vault.filter({ stability: "corrupted" }).map((item) => item.instanceId),
    ["item-2"],
  );
});
