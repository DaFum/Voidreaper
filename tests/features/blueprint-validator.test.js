...
test("a well-formed tree validates without issues", () => {
  const result = validateBlueprint(
    {
      shipFrameId: "frame-a",
      nodes: [node("root"), node("a", "root"), node("b", "a")],
    },
    options,
  );
...
test("cycles are flagged for every node caught in or leading into the cycle", () => {
  const result = validateBlueprint(
    {
      shipFrameId: "frame-a",
      nodes: [node("root"), node("a", "b"), node("b", "a"), node("c", "a")],
    },
    options,
  );
  assert.deepEqual(
    result.issues
      .filter((issue) => issue.type === "cycle")
      .map((issue) => issue.nodeId)
      .sort(),
    ["a", "b", "c"],
  );
  assert.deepEqual(
    result.blueprint.nodes.map((item) => item.blueprintNodeId),
    ["root"],
  );
});
...
test("nodes whose ancestor chain dangles at a missing parent are dropped, not kept", () => {
  ...
  assert.deepEqual(
    result.issues
      .filter((issue) => issue.type === "missing-parent")
      .map((issue) => issue.nodeId)
      .sort(),
    ["b", "c", "d"],
  );
