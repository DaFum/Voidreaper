...
test("findSurvivingSecondaryConnection returns the connection when childNodeId is the target and the source survives", () => {
  ...
  const result = findSurvivingSecondaryConnection(snapshot, "node-1");
  assert.deepEqual(result, { sourceNodeId: "node-2", targetNodeId: "node-1" });
});
...
test("findSurvivingSecondaryConnection returns the first valid connection when multiple connections touch the node, skipping invalid ones", () => {
  const snapshot = {
    secondaryConnectionsById: {
      "conn-1": { sourceNodeId: "node-1", targetNodeId: "node-2" }, // Invalid: target missing
      "conn-2": { sourceNodeId: "node-3", targetNodeId: "node-4" }, // Invalid: doesn't touch node-1
      "conn-3": { sourceNodeId: "node-1", targetNodeId: "node-5" }, // Valid: target exists
      "conn-4": { sourceNodeId: "node-6", targetNodeId: "node-1" }  // Valid, but should return conn-3
    },
    nodesById: {
      "node-1": {},
      // node-2 missing
  ...
  const result = findSurvivingSecondaryConnection(snapshot, "node-1");
  assert.deepEqual(result, { sourceNodeId: "node-1", targetNodeId: "node-5" });
});
