...
import {
  matchBlueprintNode,
  BLUEPRINT_MATCH,
} from "../../src/features/ship-assembly/blueprints/blueprint-matcher.js";

test("returns BLOCKED if occupied is true", () => {
  const target = {};
  const moduleProfile = {};
  assert.equal(
    matchBlueprintNode(target, moduleProfile, { occupied: true }),
    BLUEPRINT_MATCH.BLOCKED,
  );
});
...
test("returns INCOMPATIBLE instead of throwing when target or moduleProfile is missing", () => {
  ...
  assert.equal(matchBlueprintNode({}, null), BLUEPRINT_MATCH.INCOMPATIBLE);
  assert.equal(
    matchBlueprintNode(undefined, undefined),
    BLUEPRINT_MATCH.INCOMPATIBLE,
  );
});
...
test("returns EXACT if preferredModuleDefinitionId matches moduleProfile.definitionId", () => {
  ...
  assert.equal(
    matchBlueprintNode(target, moduleProfile),
    BLUEPRINT_MATCH.EXACT,
  );
});
