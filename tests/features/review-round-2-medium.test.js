...
} from "../../src/features/merchant/merchant-service.js";
import {
  MERCHANT_SERVICES,
  CORRUPT_OFFER,
} from "../../src/content/merchant/merchant-pools.js";
import { createRunCurrencyService } from "../../src/features/economy/run-currency-service.js";
import {
  createCorruptionSystem,
  createCorruptionState,
} from "../../src/features/corruption/corruption-system.js";
...
test("M5: reroll preserves the locked affix without growing the affix count", () => {
  const workshop = createWorkshopService({
    affixRoller: { roll: () => [{ affixId: "new-a" }, { affixId: "new-b" }] },
  });
  const session = workshop.open(0);
  const target = {
    rarity: "rare",
    itemPower: 100,
    affixes: [{ affixId: "keep-me" }, { affixId: "reroll-me" }],
  };
  assert.equal(
    workshop.apply(session, "lock", target, { affixId: "keep-me" }),
    true,
  );
  assert.equal(workshop.apply(session, "reroll", target, {}), true);
  assert.deepEqual(
    target.affixes.map((affix) => affix.affixId),
    ["keep-me", "new-a"],
  );
...
test("M7: services are priced by their content basePrice", () => {
  const stabilize = MERCHANT_SERVICES.find(
    (service) => service.id === "stabilize",
  );
...
test("M7: buying a service applies its effect instead of granting an item", () => {
  const merchant = createMerchantService({
    currencyService: createRunCurrencyService(),
  });
  const run = createMerchantRun();
  run.corruption.value = 40;
  const stabilize = {
    ...MERCHANT_SERVICES.find((service) => service.id === "stabilize"),
    offerId: "o1",
    price: 3,
  };
  ...
  const repair = {
    ...MERCHANT_SERVICES.find((service) => service.id === "repair"),
    offerId: "o2",
    price: 35,
  };
...
test("M7: the corrupted offer grants its credit alongside the corruption", () => {
  ...
  const run = createMerchantRun();
  assert.equal(
    merchant.buy(run, { ...CORRUPT_OFFER, offerId: "o3", price: 0 }),
    true,
  );
...
test("M8: capsule broadphase bounds cover the full capsule extent", () => {
  ...
  const capsule = {
    id: "z1",
    shape: { kind: "capsule", length: 46, radius: 20 },
  ...
  assert.equal(
    index.query({ minX: 39, minY: -1, maxX: 41, maxY: 1 }).length,
    1,
  );
  assert.equal(
    index.query({ minX: 50, minY: 50, maxX: 51, maxY: 51 }).length,
    0,
  );
});
...
test("M8: frame core zones expose kind like module zones", () => {
  ...
  const zones = buildAssemblyHitZones(
    snapshot,
    { coreHitZone: { shape: "capsule", length: 46, radius: 20 } },
    { onFallback: () => {} },
  );
...
test("M9: blueprint nodes record the parent port key and match ports in a new run", () => {
  const portsById = {
    "run1-port-1": { portId: "run1-port-1", key: "left-wing" },
  };
  const node = toBlueprintNode(
    {
      nodeId: "run1-node-2",
      parentNodeId: "run1-node-1",
      parentPortId: "run1-port-1",
      moduleInstanceId: "item-1",
  ...
  const newRunPort = {
    portId: "run2-port-9",
    parentNodeId: "run2-root",
    key: "left-wing",
  };
  const target = findBlueprintTarget(blueprint, newRunPort, {
    definitionId: "railgun",
  });
...
test("M13: components without disabledUntil are fault candidates", () => {
  const scheduler = createFaultScheduler({
    ...
    profiles: [
      { id: "weapon-projectile", light: ["jam"] },
      { id: "standard", light: ["glitch"] },
    ],
    faults: [{ id: "jam" }, { id: "glitch" }],
  });
  scheduler.update(0, { loadTier: "critical", heat: 100, corruption: 50 });
  const occurrence = scheduler.update(
    100,
    { loadTier: "critical", heat: 100, corruption: 50 },
    [{ id: "railgun-1", faultProfileId: "weapon-projectile" }],
  );
...
test("M14: architect overload recalculates the energy tier", () => {
  ...
  run.services = {
    ...
    heat: createHeatSystem({}),
    corruption: createCorruptionSystem({}),
  };
    ...
    rootPorts: [
      {
        ...
        localPosition: { x: -46, y: 0 },
      },
    ],
  });
  const service = createAssemblyService({
    state,
    idFactory: ids,
    runInventory: { store: () => null },
  });
  const nodeId = service.mountModule({
    moduleInstanceId: "item-1",
    definitionId: "railgun",
    parentPort: state.portsById["port-1"],
    assemblyProfile: {
      visualProfileId: "weapon-mount",
      variantSeed: 1,
      sizeClass: "M",
      mass: 6,
      damage: { armor: 10, core: 10 },
      childPorts: [],
    },
    transform: { position: { x: -46, y: 0 }, rotation: 0 },
  });
...
test("M17: previewPlacement returns a bounded rotationalInertia delta", () => {
  const flight = createFlightProfileService({
    geometryService: { getSnapshot: () => null },
  });
  const delta = flight.previewPlacement(
