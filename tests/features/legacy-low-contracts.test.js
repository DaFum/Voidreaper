...
test("legacy pause suspends audio and resume restarts it", () => {
  assert.match(
    source,
    /suspend\(\)\s*\{\s*if\s*\(this\.ctx\s*&&\s*this\.ctx\.state === "running"\)\s*this\.ctx\.suspend\(\);\s*\}/,
  );
  assert.match(source, /pause\(\).*AudioSys\.suspend\(\)/s);
});

test("glitch refresh clears the previous timeout", () => {
  const glitchStart = source.indexOf("glitch() {");
  assert.ok(glitchStart >= 0, "glitch() method found");
...
test("immediate enemy spawns skip the birth delay", () => {
  assert.match(
    source,
    /birth:\s*immediate\s*\?\s*0\s*:\s*0\.35,\s*fusing:\s*false,\s*dead:\s*false/s,
  );
});
