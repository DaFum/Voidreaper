...
describe("createRepairService", () => {
  ...
  describe("preview", () => {
    it("returns correct preview for valid node", () => {
      const result = repairService.preview(
        REPAIR_ACTIONS.PATCH_ARMOR,
        "valid-node",
      );
    ...
    it("returns allowed=true for REMOUNT_DETACHED without valid node", () => {
      const result = repairService.preview(
        REPAIR_ACTIONS.REMOUNT_DETACHED,
        "invalid-node",
      );
      assert.equal(result.allowed, true);
    });
  ...
  describe("apply", () => {
    it("throws if insufficient resources", () => {
      ...
      assert.throws(
        () => repairService.apply(REPAIR_ACTIONS.PATCH_ARMOR, "valid-node"),
        /Nicht genügend Reparaturressourcen/,
      );
    });
    ...
    it("throws if in combat without capability", () => {
      assert.throws(
        ...
        /Diese Reparatur benötigt ein aktives Reparaturmodul/,
      );
    });

    it("handles REPLACE_PORT successfully", () => {
      const result = repairService.apply(
        REPAIR_ACTIONS.REPLACE_PORT,
        "valid-port",
    ...
    it("throws on REPLACE_PORT with unknown port", () => {
      assert.throws(
        () => repairService.apply(REPAIR_ACTIONS.REPLACE_PORT, "invalid-port"),
        /Unknown assembly port: invalid-port/,
      );
    });
    ...
    it("throws on REMOUNT_DETACHED if remount function is unavailable", () => {
      ...
      assert.throws(
        () =>
          serviceNoRemount.apply(REPAIR_ACTIONS.REMOUNT_DETACHED, "node-id"),
        /Detached module remount is unavailable/,
      );
    });
    ...
    it("handles PATCH_ARMOR successfully", () => {
      ...
      assert.equal(
        mockEventBus.events[0].payload.action,
        REPAIR_ACTIONS.PATCH_ARMOR,
      );
    });
    ...
    it("handles STABILIZE_BRACE successfully", () => {
      const result = repairService.apply(
        REPAIR_ACTIONS.STABILIZE_BRACE,
        "valid-node",
      );
