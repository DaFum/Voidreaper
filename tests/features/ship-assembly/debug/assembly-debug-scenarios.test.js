...
} from "../../../../src/features/ship-assembly/debug/assembly-debug-scenarios.js";
...
test("maximum construction skips candidates rejected by compatibility", () => {
  ...
  const candidate = findMaximumConstructionCandidate({
    ...
    evaluate: ({ moduleProfile }) => ({
      compatible: moduleProfile.definitionId === "good",
    }),
  });
...
test("maximum construction prioritizes an expandable definition across all ports", () => {
  ...
  const candidate = findMaximumConstructionCandidate({
    ...
    geometry: {},
    ports: [
      { portId: "first", branchDepth: 0 },
      { portId: "second", branchDepth: 0 },
    ],
    definitions: [leaf, spine],
    evaluate: ({ moduleProfile, port }) => ({
      compatible:
        moduleProfile.definitionId === "leaf" || port.portId === "second",
    }),
  });
