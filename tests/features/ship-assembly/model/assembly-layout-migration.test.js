...
test("checkpoint assemblies receive canonical root and child port transforms", () => {
  const state = {
    ...
    nodesById: {
      ...
      leaf: {
        ...
        definitionId: "leaf",
        parentNodeId: "module",
        parentPortId: "child-port",
        localPosition: { x: 0, y: -34 },
        localRotation: 0,
      },
    },
    portsById: {
      ...
      "child-port": {
        ...
        direction: { x: 0, y: -1 },
        localPosition: { x: 0, y: -34 },
        occupiedByNodeId: "leaf",
      },
    },
  };
  const equipment = {
    requireAssemblyProfile: () => ({
      childPorts: [
        {
          key: "micro-port",
          direction: { x: 0, y: 1 },
          localPosition: { x: 0, y: 42 },
        },
      ],
    }),
  };
  ...
  assert.deepEqual(state.portsById["root-port"].localPosition, {
    x: -72,
    y: 0,
  });
  assert.deepEqual(state.nodesById.module.localPosition, { x: -72, y: 0 });
  assert.deepEqual(state.portsById["child-port"].localPosition, {
    x: 0,
    y: 42,
  });
