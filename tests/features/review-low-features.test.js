...
test("merchant offers can only be purchased once", () => {
  ...
  assert.equal(
    merchant.roll(run, 7).some((entry) => entry.offerId === offer.offerId),
    false,
  );
  assert.deepEqual(JSON.parse(JSON.stringify(run)).consumedOfferIds, [
    offer.offerId,
  ]);
  ...
  assert.equal(
    merchant.roll(nextRun, 7).some((entry) => entry.offerId === offer.offerId),
    true,
  );
});
...
test("weapon telemetry omits an unsupported hardcoded damage figure", () => {
  ...
  const adapter = {
    ...
    onEquip() {},
    onUnequip() {},
    getTelemetry: () => ({ ready: true }),
  };
  controller.equip(
    { id: "test-weapon", adapter },
    { player: {}, enemies: [], rng: {} },
  );
...
test("corruption bookings aggregate by source instead of growing per change", () => {
  ...
  for (let index = 0; index < 100; index += 1)
    system.change(state, 1, "abyss", index, { allowAbyss: true });
...
test("heat warning survives a frame hitch that expires cooling delay", () => {
  ...
  const system = createHeatSystem({
    eventBus: { emit: (event) => warnings.push(event) },
  });
...
test("flight placement preview prefers world coordinates for nested ports", () => {
  const service = createFlightProfileService({
...
test("repair preserves the canonical detached damage state", () => {
  ...
  const repairs = createRepairService({
    assemblyService,
