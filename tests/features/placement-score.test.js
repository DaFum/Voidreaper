...
import {
  explainPlacement,
  scorePlacement,
} from "../../src/features/ship-assembly/placement/placement-score.js";
...
test("explainPlacement", async (t) => {
  ...
  await t.test("identifies Beste Waffenlinie", () => {
    const reasons = explainPlacement(
      { ...defaultMetrics, fireLane: 0.75 },
      defaultDelta,
    );
  ...
  await t.test("identifies Gut geschützt", () => {
    const reasons = explainPlacement(
      { ...defaultMetrics, protection: 0.75 },
      defaultDelta,
    );
  ...
  await t.test("identifies Erhöht Trägheit from delta", () => {
    const reasons = explainPlacement(defaultMetrics, {
  ...
  await t.test(
    "does not identify reasons at exact threshold boundaries",
    () => {
...
test("scorePlacement", async (t) => {
  await t.test("calculates score correctly based on weights", () => {
    ...
    assert.ok(
      Math.abs(score - 4.5) < 0.0001,
      `Score was ${score}, expected 4.5`,
    );
  });
});
