...
import {
  generateSectorMap,
  flattenSectorMap,
} from "../../../src/features/sectors/sector-map-generator.js";

test("sector-map-generator", async (t) => {
  await t.test(
    "generateSectorMap creates a valid sector map with default arguments",
    () => {
  ...
  await t.test(
    "generateSectorMap falls back to default regions when campaignPathId is invalid",
    () => {
  ...
  await t.test(
    "generateSectorMap avoids merchant-to-merchant connections",
    () => {
      ...
      for (let seed = 0; seed < 10; seed++) {
        ...
        for (const merchant of merchants) {
          ...
          for (const nextId of merchant.next) {
            ...
            assert.ok(
              nextNode,
              "Connected node " + nextId + " should exist in the map",
            );
  ...
  await t.test(
    "generateSectorMap forces mid-boss or extraction on layer 2",
    () => {
  ...
  await t.test(
    "generateSectorMap assigns appropriate boss to region 4 vs others",
    () => {
  ...
  await t.test(
    "flattenSectorMap flattens regions and layers and caches result",
    () => {
