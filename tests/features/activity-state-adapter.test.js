...
test("buildModuleActivityState", async (t) => {
  await t.test(
    "returns default fallback values when no argument is provided",
    () => {
  ...
  await t.test(
    "converts truthy and falsy values to booleans for firing and faulting",
    () => {
  ...
  await t.test(
    "handles null telemetry gracefully by falling back to defaults",
    () => {
