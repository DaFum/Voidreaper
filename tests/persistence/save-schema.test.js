...
} from "../../src/persistence/save-schema.js";
...
test("createDefaultSave creates a save object matching meta defaults", () => {
  ...
  assert.equal(
    save.saveVersion,
    CURRENT_SAVE_VERSION,
    "saveVersion should match CURRENT_SAVE_VERSION",
  );
  assert.equal(
    typeof save.profile.createdAt,
    "string",
    "Should generate a creation date string",
  );
  ...
  assert.equal(
    save.unlocks["standard-core"],
    true,
    "Should force unlock standard-core",
  );
  ...
  assert.deepEqual(
    save.migrationHistory,
    [],
    "Migration history should be empty array",
  );
  assert.deepEqual(
    save.migrationBackups,
    {},
    "Migration backups should be empty object",
  );
});
