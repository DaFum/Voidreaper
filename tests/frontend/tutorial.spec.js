...
} from "../../src/ui/components/tutorial-overlay.js";
...
describe("tutorial overlay", () => {
    ...
    try {
      ...
      overlay.render({
        active: {
          ...
          step: { kind: "action", title: "Karte", body: "Öffne sie." },
          stepIndex: 0,
          stepCount: 1,
        },
      });
  ...
  test("maps every combat tutorial target to the real HUD containers", () => {
    ...
    expect(document.querySelector("#game").dataset.tutorialId).toBe(
      "game-canvas",
    );
    expect(document.querySelector("#combat-actions").dataset.tutorialId).toBe(
      "hud-active-modules",
    );
    expect(document.querySelector("#resumebtn").dataset.tutorialId).toBe(
      "pause-resume",
    );
    expect(document.querySelector("#cards").dataset.tutorialId).toBe(
      "levelup-options",
    );
  });
  ...
  test("full-bleed targets pin the card to the corner instead of centering it", () => {
    ...
    const position = placeTutorialCard(
      { top: 0, bottom: 872, left: 0, right: 800, width: 800, height: 872 },
  ...
  test("re-resolves targets automatically when navigation replaces the screen", async () => {
    ...
    expect(root.querySelector('[data-role="status"]').textContent).toContain(
      "nicht sichtbar",
    );
    ...
    renderTutorialLibrary(root, {
      chapters: [
        ...
        },
        {
          ...
          description: "C",
          available: false,
          lockedReason: "Erst entdecken",
        },
      ],
      onStart,
    });
    expect(root.textContent).toContain("STARTEN");
    expect(root.textContent).toContain("WIEDERHOLEN");
    expect(root.textContent).toContain("Erst entdecken");
    root.querySelector('[data-chapter-id="a"]').click();
    expect(onStart).toHaveBeenCalledWith("a", "guided");
  });
});
