...
import {
  ASSEMBLY_VIEW_MODES,
  ASSEMBLY_VIEW_MODE_LABELS,
  getViewModeOverlay,
  renderAssemblyToolbar,
  renderViewModeOverlay,
} from "../../src/ui/ship-assembly/assembly-view-modes.js";
...
import {
  isBlueprintDetailAction,
  renderBlueprintDetail,
} from "../../src/ui/ship-assembly/blueprint-detail-screen.js";
import { renderBlueprintGhostOverlay } from "../../src/ui/ship-assembly/blueprint-ghost-overlay.js";
import {
  blueprintImportErrorMessage,
  createBlueprintImportDialog,
} from "../../src/ui/ship-assembly/blueprint-import-dialog.js";
...
import {
  portAccessibilityLabel,
  renderPortOverlay,
} from "../../src/ui/ship-assembly/port-overlay.js";
...
describe("assembly canvas controller", () => {
  ...
  test("screenToWorld converts through zoom and offset", () => {
    ...
    const controller = createAssemblyCanvasController({
      canvas,
      camera: { rotation: 0, zoom: 2, offset: { x: 5, y: -5 } },
    });
...
describe("assembly inspector panel", () => {
  ...
  test("renders node details, deltas, warnings and enabled actions", () => {
    const panel = root();
    renderAssemblyInspector(panel, {
      node: { definitionId: "core-x" },
      deltas: [["MASSE", 12]],
      warnings: ["Überlast"],
      actions: { rotate: true, dismantle: true },
    });
    ...
    expect(panel.querySelector('[data-action="move-branch"]').disabled).toBe(
      true,
    );
    expect(panel.querySelector('[data-action="dismantle"]').disabled).toBe(
      false,
    );
  });
...
describe("assembly touch controls", () => {
  ...
  const touch = (type, x, y) =>
    Object.assign(new Event(type), { clientX: x, clientY: y });
  ...
  test("movement cancels the long press and detach removes all handlers", () => {
    ...
    const element = root(),
      onDetails = vi.fn(),
      onSelect = vi.fn();
    const detach = attachAssemblyTouchControls(element, {
      onDetails,
      onSelect,
    });
    element.dispatchEvent(touch("touchstart", 50, 50));
...
describe("assembly view modes", () => {
  test("toolbar renders every mode with pressed state and reports clicks", () => {
    ...
    expect(buttons.map((button) => button.dataset.mode)).toEqual(
      Object.values(ASSEMBLY_VIEW_MODES),
    );
    expect(
      buttons
        .find((button) => button.dataset.mode === "energy")
  ...
  test("getViewModeOverlay derives per-mode overlays from the geometry snapshot", () => {
    ...
    const structure = getViewModeOverlay(ASSEMBLY_VIEW_MODES.STRUCTURE, {
      assembly,
      geometry,
    });
    ...
    const flight = getViewModeOverlay(ASSEMBLY_VIEW_MODES.FLIGHT, {
      flightProfile: {
        centerOfMass: { x: 1, y: 2 },
        thrustVectors: [],
        totalMass: 88,
      },
    });
  ...
  test("renderViewModeOverlay draws connections, labels, center of mass and thrust vectors", () => {
    ...
    renderViewModeOverlay(ctx, {
      connections: [
        { cable: { from: { x: 0, y: 0 }, to: { x: 10, y: 0 } }, label: "high" },
        { spine: {} },
      ],
      labels: [{ position: { x: 1, y: 1 }, text: "KERN" }],
      centerOfMass: { x: 3, y: 3 },
      thrustVectors: [
        { position: { x: 0, y: 0 }, direction: { x: 0, y: 1 }, strength: 1 },
      ],
    });
    expect(ctx.count("stroke")).toBe(3);
    expect(
      ctx.calls.filter(([name]) => name === "fillText").map((call) => call[1]),
    ).toEqual(["high", "KERN", "COM"]);
...
describe("assembly workbench screen", () => {
  test("builds the workbench layout and forwards data-action clicks until destroyed", () => {
    ...
    expect(
      container.querySelector('[aria-label="Schiffswerkbank"]'),
    ).not.toBeNull();
  ...
  test("renderInventory lists items with selection state and an empty fallback", () => {
    ...
    const buttons = [
      ...screen.root.querySelectorAll('[data-action="select-item"]'),
    ];
  ...
  test("setHint updates the live region and setInspector swaps panel content", () => {
    ...
    expect(
      screen.root.querySelector('[data-role="inspector"]').textContent,
    ).toBe("Inspector");
  });
});
...
describe("blueprint card", () => {
  const blueprint = {
    ...
    nodes: [{}, {}],
    favorite: false,
    usage: { highestAbyssDepth: 4 },
  };

  test("renders metadata, rejects unsafe thumbnails and wires open/favorite", () => {
    const onOpen = vi.fn(),
      onFavorite = vi.fn();
    const card = createBlueprintCard(
      { ...blueprint, thumbnailDataUrl: "javascript:alert(1)" },
      { onOpen, onFavorite },
    );
  ...
  test("accepts safe data-url thumbnails and reflects the favorite state", () => {
    ...
    expect(
      card
        .querySelector(".blueprint-card__favorite")
        .getAttribute("aria-pressed"),
    ).toBe("true");
  });
});
...
describe("blueprint detail screen", () => {
  ...
  test("renders all actions, disables activate for the active template and reports actions", () => {
    ...
    expect(container.querySelector('[data-action="activate"]').disabled).toBe(
      true,
    );
...
describe("blueprint ghost overlay", () => {
  test("draws one triangle per ghost with match-dependent alpha", () => {
    ...
    renderBlueprintGhostOverlay(ctx, [
      { position: { x: 0, y: 0 }, rotation: 0.3, size: 10, match: "exact" },
...
describe("blueprint import dialog", () => {
  test("maps codec errors to an actionable message", () => {
    ...
    ).toBe("Ungültiges Bauplanformat");
    expect(blueprintImportErrorMessage(new Error("boom"))).toBe(
      "Der Bauplan-Code ist beschädigt oder unvollständig.",
    );
    expect(blueprintImportErrorMessage(null)).toBe(
      "Der Bauplan-Code ist beschädigt oder unvollständig.",
    );
  });
  ...
  test("rejects broken codes and keeps the import button disabled", () => {
    const container = root();
    createBlueprintImportDialog(container, {
      validate: () => ({ valid: true, issues: [] }),
    });
    container.querySelector("textarea").value = "kein base64url!!";
    container.querySelector('[data-action="inspect"]').click();
    expect(
      container.querySelector('[data-role="report"]').textContent,
    ).toContain("Bauplanformat");
    expect(container.querySelector('[data-action="import"]').disabled).toBe(
      true,
    );
  });
  ...
  test("validates a real encoded blueprint, lists issues and imports the validated result", () => {
    ...
    const validate = vi.fn(() => ({
      valid: true,
      issues: [{ type: "unknown-definition", definitionId: "old-mod" }],
      blueprint: decoded,
    }));
    const onImport = vi.fn();
    createBlueprintImportDialog(container, { validate, onImport });
    container.querySelector("textarea").value = encodeBlueprint({
      blueprintVersion: 3,
      shipFrameId: "frame-a",
      nodes: [],
      connections: [],
    ...
    expect(validate).toHaveBeenCalledWith(
      expect.objectContaining({ shipFrameId: "frame-a" }),
    );
    expect(
      container.querySelector('[data-role="report"]').textContent,
    ).toContain("VALIDIERT");
    expect(
      container.querySelector('[data-role="report"]').textContent,
    ).toContain("unknown-definition: old-mod");
  ...
  test("editing the code after validation invalidates the stale result", () => {
    ...
    const onImport = vi.fn();
    createBlueprintImportDialog(container, {
      validate: () => ({ valid: true, issues: [], blueprint: decoded }),
      onImport,
    });
  ...
  test("keeps invalid results non-importable", () => {
    const container = root();
    createBlueprintImportDialog(container, {
      validate: () => ({ valid: false, issues: [{ type: "missing-root" }] }),
    });
    container.querySelector("textarea").value = encodeBlueprint({
      blueprintVersion: 3,
      shipFrameId: null,
...
describe("blueprint library screen", () => {
  test("renders a card per blueprint and wires import/create", () => {
    const container = root(),
      ...
      onCreate = vi.fn(),
      onOpen = vi.fn();
    renderBlueprintLibrary(container, {
      blueprints: [
        { blueprintId: "bp-1", name: "Eins", shipFrameId: "f", nodes: [] },
      ],
      onImport,
      onCreate,
      onOpen,
...
describe("placement preview overlay", () => {
  test("clears the canvas, renders the ship and highlights the suggested transform", () => {
    ...
    renderPlacementPreview(ctx, {
      snapshot: { id: "snap" },
      suggestion: { transform: { position: { x: 10, y: 5 }, rotation: 1 } },
      assemblyRenderer,
      time: 2,
    });
    expect(ctx.count("clearRect")).toBe(1);
    expect(assemblyRenderer.renderPlayerShip).toHaveBeenCalledWith(
      ctx,
      expect.objectContaining({ geometrySnapshot: { id: "snap" } }),
    );
  ...
  test("skips the suggestion marker without a transform", () => {
    ...
    renderPlacementPreview(ctx, {
      snapshot: {},
      suggestion: null,
      assemblyRenderer: { renderPlayerShip: () => {} },
    });
...
describe("port overlay", () => {
  test("accessibility label describes size, mount, energy, load and state", () => {
    ...
    expect(portAccessibilityLabel(port, null)).toBe(
      "M-Port, hardpoint, Energie high, Traglast 6, verfügbar",
    );
    expect(
      portAccessibilityLabel({ ...port, occupiedByNodeId: "n1" }, null),
    ).toContain("belegt");
    expect(
      portAccessibilityLabel(port, {
        compatible: false,
        reasonLabels: ["zu groß"],
      }),
    ).toContain("inkompatibel: zu groß");
  });
  ...
  test("renders each port and dims incompatible ones", () => {
    ...
    renderPortOverlay(
      ...
      {
        selectedPortId: "p1",
        compatibilityByPortId: { p2: { compatible: false } },
      },
    );
    expect(ctx.count("save")).toBe(ctx.count("restore"));
    expect(alphas).toContain(0.27);
...
describe("quick mount overlay", () => {
  ...
  test("render fills name, reasons, capped deltas and details", () => {
    ...
    overlay.render({
      name: "Void Coil",
      reasons: ["Grund A", "Grund B", "Grund C"],
    ...
    expect(
      overlay.root.querySelector('[data-role="module-name"]').textContent,
    ).toBe("Void Coil");
    expect(overlay.root.querySelector('[data-role="reason"]').textContent).toBe(
      "Grund A · Grund B",
    );
    expect(
      overlay.root.querySelectorAll('[data-role="deltas"] div'),
    ).toHaveLength(6);
    expect(
      overlay.root.querySelector('[data-role="details"]').textContent,
    ).toBe("D1 · D2");
  });
