...
import {
  createBuildInspector,
  describeUpgradeImpact,
} from "../../src/ui/screens/build-inspector.js";
...
import {
  renderBuildHistory,
  renderCodexScreen,
} from "../../src/ui/screens/codex-screen.js";
...
} from "../../src/ui/screens/merchant-screen.js";
...
import {
  ...
  workshopDisabledReason,
} from "../../src/ui/screens/workshop-screen.js";
...
describe("abyss transition screen", () => {
  test("renders the profile and wires descend/extract", () => {
    const container = root(),
      onExtract = vi.fn(),
      onDescend = vi.fn();
    renderAbyssTransition(container, {
      profile: {
        depth: 3,
    ...
    renderAnomalyScreen(container, {
      event: {
        ...
        choices: [
          ...
          { id: "leave", label: "Gehen", cost: "0", reward: "-", unknown: "-" },
        ],
      },
      onChoose,
    });
...
describe("build inspector", () => {
  const model = {
    ...
    tags: Object.assign(new Map([["Void", 4]]), {
      provenance: new Map([["Void", [{ sourceId: "mod-a" }]]]),
    }),
    evolutions: [
      {
        definition: { name: "Evo", kind: "weapon" },
        eligible: true,
        requirements: [{ met: true }, { met: false }],
        blockedBy: null,
      },
    ],
    load: { ratio: 0.9, tier: "strained" },
    heat: 42,
    corruption: 17,
    faultPressure: 0.5,
    statContext: {},
  ...
  const services = {
    stats: {
      definitions: () => [
        { id: "dps", displayName: "DPS", displayFormat: "integer" },
      ],
      calculate: () => ({ value: 10, baseValue: 8, contributions: [] }),
    },
    tags: { delta: vi.fn(() => "tag-delta") },
  };
  ...
  test("renders synergies by default and switches tabs on click", () => {
    ...
    expect(container.querySelector(".inspector-panel").innerHTML).toContain(
      "Aktiv",
    );
  ...
  test("describeUpgradeImpact combines tag delta and before/after values", () => {
    const impact = describeUpgradeImpact(
      { sources: [1], load: "L1", heat: 1, corruption: 2 },
      { sources: [2], load: "L2", heat: 3, corruption: 4 },
      services,
    );
    expect(impact).toEqual({
      tags: "tag-delta",
      load: { before: "L1", after: "L2" },
      heat: { before: 1, after: 3 },
      corruption: { before: 2, after: 4 },
    });
  });
});
...
describe("challenges screen", () => {
  test("counts claimed challenges and marks mastery state", () => {
    ...
    expect(articles.map((article) => article.dataset.level)).toEqual([
      "mastered",
      "observed",
    ]);
...
describe("codex screen", () => {
  test("renders entries, discovery counter and accessible filters", () => {
    ...
    expect(onFilter).toHaveBeenCalledWith(
      expect.objectContaining({ tag: "Void" }),
    );
  });
...
describe("extraction screen", () => {
  test("renders window progress and wires hold/cancel", () => {
    const container = root(),
      onHold = vi.fn(),
      onCancel = vi.fn();
    renderExtractionScreen(container, {
      window: { reason: "boss", marked: [1, 2], duration: 10, elapsed: 5 },
      onHold,
      onCancel,
    });
    ...
    expect(
      container.querySelector('[data-tutorial-id="extraction-options"]'),
    ).not.toBeNull();
    expect(container.querySelector(".bar i").style.transform).toBe(
      "scaleX(0.5)",
    );
...
describe("hangar screen", () => {
  const catalogs = {
    ships: [{ id: "ship-1", slot: "ship", name: "Frame" }],
    weapons: [],
    modules: [],
    reactors: [],
  };
  const catalogLoadout = {
    slots: {
      ...
      passive: [
        { definitionId: "mod-equipped" },
        { definitionId: "mod-old" },
        null,
        null,
      ],
      active: [null, null],
  ...
  test("maps every catalog unlock source to player-facing copy", () => {
    ...
    expect(catalogUnlockLabel("blueprint")).toBe(
      "Durch Blaupause freischalten",
    );
    expect(catalogUnlockLabel("challenge")).toBe(
      "Über Herausforderung freischalten",
    );
  ...
  test("derives equipped, available and locked entries in the required order", () => {
    const entries = [
      ...
      },
      {
        ...
        state: "equipped",
        equippedSlots: [{ slot: "passive", index: 0 }],
      },
    ];
    ...
    expect(filtered.map((entry) => entry.state)).toEqual([
      "equipped",
      "available",
      "locked",
    ]);
  ...
  test("starts on the launch tab and fires onStart/onResume", () => {
    ...
    createHangarScreen(container, {
      ...catalogs,
      currencies: { voidShards: 3 },
      checkpoint: { nodeId: "cp-1" },
      onStart,
      onResume,
    });
    expect(
      container.querySelector('[role="tab"][aria-selected="true"]').dataset
        .hangarTab,
    ).toBe("Run starten");
  ...
  test("shows catalog orientation, visible lock state and stable default order", () => {
    ...
    expect(
      container.querySelector("[data-catalog-title]").textContent,
    ).toContain("Schiffe");
    expect(
      container.querySelector("[data-catalog-progress]").textContent,
    ).toContain("2 von 3 freigeschaltet");
    expect(
      [...container.querySelectorAll(".item-card")].map(
        (card) => card.dataset.itemId,
      ),
    ).toEqual(["ship-equipped", "ship-open", "ship-locked"]);
    expect(
      container.querySelector('[data-item-id="ship-locked"]').textContent,
    ).toContain("GESPERRT");
    expect(
      container.querySelector('[data-item-id="ship-locked"]').textContent,
    ).toContain("Über Forschung freischalten");
  });
  ...
  test("does not count an equipped but locked definition as unlocked", () => {
    ...
    expect(container.querySelector("[data-catalog-progress]").textContent).toBe(
      "0 von 1 freigeschaltet",
    );
    expect(
      container.querySelector('[data-item-id="legacy-ship"]').dataset.state,
    ).toBe("equipped");
  });
  ...
  test("closes a selection when filters hide its card", () => {
    ...
    expect(container.querySelector("[data-catalog-selection]").hidden).toBe(
      false,
    );
  ...
  test("opens current matching slots and equips only the explicitly chosen slot", async () => {
    const container = root(),
      onEquip = vi.fn().mockResolvedValue({ ok: true });
    const screen = createHangarScreen(container, {
      ships: [],
      weapons: [],
      reactors: [],
    ...
    container.querySelector('[data-item-id="mod-new"]').click();
    const reopenedSlots = [
      ...container.querySelectorAll("[data-catalog-equip]"),
    ];
    reopenedSlots[1].click();
    await vi.waitFor(() =>
      expect(onEquip).toHaveBeenCalledWith("passive", 1, "mod-new"),
    );
  });
  ...
  test("keeps locked details inspectable but never offers or dispatches equip", () => {
    ...
    const screen = createHangarScreen(container, {
      ships: [
        { id: "locked", slot: "ship", name: "Locked", unlockSource: "secret" },
      ],
      weapons: [],
      modules: [],
      reactors: [],
    ...
    expect(
      container.querySelector("[data-catalog-selection]").textContent,
    ).toContain("Geheime Bedingung erfüllen");
  ...
  test("keeps the slot chooser open and announces a rejected equip result", async () => {
    ...
    const screen = createHangarScreen(container, {
      ships: [
        { id: "vesper", slot: "ship", name: "Vesper", unlockSource: "starter" },
      ],
      weapons: [],
      modules: [],
      reactors: [],
      loadout: catalogLoadout,
      isUnlocked: () => true,
      onEquip: vi.fn().mockResolvedValue({
        ok: false,
        message: "Vesper kann nicht ausgerüstet werden",
      }),
    });
    screen.show("Schiffe");
    ...
    await vi.waitFor(() =>
      expect(
        container.querySelector("[data-catalog-message]").textContent,
      ).toContain("kann nicht ausgerüstet werden"),
    );
    expect(container.querySelector("[data-catalog-selection]").hidden).toBe(
      false,
    );
  ...
  test("switches tabs by click, renders catalogs with lock state and delegates unknown tabs", () => {
    ...
    screen.show("Codex");
    expect(container.querySelector(".hangar-stage").dataset.activeTab).toBe(
      "Codex",
    );
  ...
  test("provides desktop overflow controls and keyboard tab navigation", () => {
    ...
    expect(
      container
        .querySelector('[data-hangar-scroll="previous"]')
        .getAttribute("aria-label"),
    ).toBe("Vorherige Bereiche anzeigen");
    expect(
      container
        .querySelector('[data-hangar-scroll="next"]')
        .getAttribute("aria-label"),
    ).toBe("Weitere Bereiche anzeigen");
  ...
  test("mobile area picker uses the shared tab path and restores focus", () => {
    ...
    container
      .querySelector("[data-hangar-area-panel]")
      .dispatchEvent(
        new KeyboardEvent("keydown", { key: "Escape", bubbles: true }),
      );
    expect(document.activeElement).toBe(
      container.querySelector("[data-hangar-area-toggle]"),
    );
    container.remove();
  });
  ...
  test("refreshes overflow controls when the hidden start hangar becomes visible", async () => {
    ...
    await new Promise((resolve) => requestAnimationFrame(resolve));
    expect(
      container.querySelector('[data-hangar-scroll="next"]').disabled,
    ).toBe(false);
    start.remove();
  });
  ...
  test("positions the active tab after layout and refreshes overflow controls", async () => {
    ...
    Object.defineProperty(HTMLElement.prototype, "clientWidth", {
      configurable: true,
      get() {
        return layoutReady && this.classList?.contains("hangar-tabs") ? 300 : 0;
      },
    });
    Object.defineProperty(HTMLElement.prototype, "scrollWidth", {
      configurable: true,
      get() {
        return layoutReady && this.classList?.contains("hangar-tabs") ? 900 : 0;
      },
    });
    Object.defineProperty(HTMLElement.prototype, "offsetLeft", {
      configurable: true,
      get() {
        return this.dataset?.hangarTab === "Einstellungen" ? 800 : 0;
      },
    });
    Object.defineProperty(HTMLElement.prototype, "offsetWidth", {
      configurable: true,
      get() {
        return this.dataset?.hangarTab === "Einstellungen" ? 100 : 0;
      },
    });
    try {
      const container = root();
      ...
      expect(
        container.querySelector('[data-hangar-scroll="previous"]').disabled,
      ).toBe(false);
    } finally {
...
describe("loadout screen", () => {
  ...
  test("loadoutStatus reports unconfigured loadouts explicitly", () => {
    expect(
      loadoutStatus({ sources: [], load: { ratio: 9, tier: "collapse" } }),
    ).toEqual({ percent: 0, tier: "unconfigured" });
  ...
  test("maps equipped definition ids to their player-facing names", () => {
    expect(loadoutDefinitionNames(inspection)).toEqual(
      new Map([
        ["frame-x", "Frame X"],
        ["mod-1", "Module One"],
      ]),
  ...
  test("renders frame, slot grid, telemetry and blueprint selection", () => {
    ...
    renderLoadoutScreen(
      container,
      inspection,
      { slots: { passive: [{ definitionId: "mod-1" }] } },
      {
        blueprints: [{ blueprintId: "bp-1", name: "Vorlage" }],
    ...
    expect(
      container.querySelector('[data-slot="passive"] strong').textContent,
    ).toBe("Module One");
    expect(
      container
        .querySelector('[data-slot="passive"]')
        .getAttribute("aria-label"),
    ).toContain("Module One");
  ...
  test("renders tags from the tag engine inspection shape", () => {
    ...
    renderLoadoutScreen(
      ...
      {
        ...inspection,
        tags: { totals: new Map([["Void", 2]]), provenance: new Map() },
      },
      { slots: {} },
    );
  ...
  test("slot picker reports equip and unequip actions", () => {
    ...
    const loadout = {
      slots: {
        ship: [{ instanceId: "starter-vesper", definitionId: "vesper" }],
      },
    };
  ...
  test("empty slot picker explains recovery paths and navigates through the hangar", () => {
    ...
    renderLoadoutScreen(
      ...
      inspection,
      { slots: {} },
      { choicesBySlot: {}, onNavigate },
    );
    container.querySelector('[data-slot="passive"]').click();
    expect(
      container.querySelector("[data-picker-empty]").textContent,
    ).toContain("Forschung oder Bergung");
...
describe("merchant screen", () => {
  test("affordability follows the offer currency and corruption bypasses it", () => {
    ...
    expect(
      canAffordOffer({ scrap: 0, flux: 6 }, { currency: "flux", price: 6 }),
    ).toBe(true);
    expect(
  ...
  test("disables unaffordable offers with an explanatory label and wires buy/reroll/leave", () => {
    const container = root(),
      ...
      onLeave = vi.fn();
    const offers = [
      { name: "Teuer", price: 50, slot: "module" },
      { name: "Billig", price: 1, slot: "module" },
    ];
    renderMerchantScreen(container, {
      offers,
      resources: { scrap: 10, flux: 0 },
      onBuy,
      onReroll,
      onLeave,
    });
    const [expensive, cheap] = [
      ...container.querySelectorAll(".item-catalog .item-card"),
    ];
    expect(expensive.disabled).toBe(true);
    expect(expensive.getAttribute("aria-label")).toContain(
      "nicht genügend Scrap",
    );
    cheap.click();
...
describe("prototype vault screen", () => {
  const items = [
    ...
    { instanceId: "i2", name: "Zwei", rarity: "common", favorite: false },
  ];
  ...
  test("renders items with accessible favorite toggles and dismantle lock for favorites", () => {
    const container = root(),
      onFavorite = vi.fn(),
      onDismantle = vi.fn();
    renderPrototypeVault(container, {
      items,
      capacity: 20,
      overflowCount: 1,
      onFavorite,
    ...
    expect(container.querySelector('[data-dismantle="i1"]').disabled).toBe(
      true,
    );
    favorite.click();
...
describe("research screen", () => {
  ...
  test("classifies nodes as owned/available/locked and purchases only enabled nodes once", () => {
    ...
    renderResearchScreen(container, nodes, {
      purchased: { r1: true },
      canPurchase: (node) => node.id === "r2",
      onPurchase,
    });
    const states = [...container.querySelectorAll(".research-node")].map(
...
describe("salvage mission screen", () => {
  test("renders the carried prototype, affixes, path and start action", () => {
    const container = root(),
      onStart = vi.fn();
    renderSalvageMission(
      container,
      {
        regionId: "grave-circuit",
        boss: { carriedItem: { name: "Original" } },
        enemyAffixes: [{ id: "fast" }],
        path: [{ type: "combat", seed: 7 }],
      },
      onStart,
    );
...
describe("sector map screen", () => {
  ...
  const model = {
    map: {
      regions: [
        { layers: [[node("start", 0, { next: ["mid"] })], [node("mid", 1)]] },
      ],
    },
    regionIndex: 0,
    currentNodeId: null,
    visitedNodeIds: ["start"],
    reachableNodeIds: ["mid"],
  };
  ...
  test("first tap selects with details, second tap confirms the reachable node", () => {
    ...
    expect(container.querySelector('[data-node-id="start"]').disabled).toBe(
      true,
    );
    ...
    expect(
      container.querySelector(".sector-map__detail").textContent,
    ).toContain("erneut tippen zum Bestätigen");
    reachable().click();
  ...
  test("sector nodes expose region, danger, and reward as responsive text units", () => {
    ...
    expect(button.querySelector(".sector-node__danger").textContent).toContain(
      "Gefahr 3",
    );
    expect(button.querySelector(".sector-node__reward").textContent).toContain(
      "Prototyp-Chance",
    );
  });
...
describe("sector summary screen", () => {
  test("sorts damage sources descending and counts telemetry", () => {
    const container = root();
    renderSectorSummary(container, {
      damageBySource: { laser: 10, rail: 90 },
      heatPeaks: [1],
      faults: [],
      synergies: [1, 2],
      evolutionProgress: [],
      codexSignatures: [1],
      prototypes: [{ secured: true }, { secured: false }],
    });
...
describe("settings screen", () => {
  const makeSettings = () => ({
    ...
    colorPatterns: false,
    uiScale: 1,
    bindings: { Space: "dodge", KeyQ: "active-1", KeyE: "active-2" },
  });
  ...
  test("exposes stable focus targets for every accessibility tutorial step", () => {
    ...
    for (const id of [
      ...
      expect(
        container.querySelector(`[data-tutorial-id="${id}"]`),
        id,
      ).not.toBeNull();
    }
  });
  ...
  test("renders toggles from settings and persists checkbox changes", () => {
    ...
    const reducedMotion = container.querySelector(
      '[data-setting="reducedMotion"]',
    );
    expect(reducedMotion.checked).toBe(false);
    expect(
      container.querySelector('[data-setting="screenShake"]').checked,
    ).toBe(true);
  ...
  test("rebinding to a key owned by another action swaps the two bindings", () => {
    ...
    expect(container.querySelector('[data-binding="active-2"]').value).toBe(
      "Space",
    );
...
describe("simulator screen", () => {
  ...
  test("prefills the config and collects the form into onStart", () => {
    const container = root(),
      onStart = vi.fn();
    renderSimulatorScreen(container, {
      config: { enemyId: "armored", density: 2, duration: 90, seed: 5 },
      summary: { dps: 12.34, triggers: 9, faults: [], seed: 5 },
      onStart,
    });
...
describe("workshop screen", () => {
  ...
  test("explains why an action is disabled", () => {
    ...
    expect(workshopDisabledReason(session, { allowed: false, points: 4 })).toBe(
      "nicht genügend Aktionspunkte",
    );
    expect(workshopDisabledReason(session, { allowed: false, points: 1 })).toBe(
  ...
  test("previews an action and executes it only after confirmation", async () => {
    ...
    document.body.append(container);
    const service = {
      preview: (currentSession, id) => ({
        allowed: id === "swap",
        points: 2,
        consequence: `Folge ${id}`,
      }),
    };
    renderWorkshopScreen(container, {
      service,
      session,
      target: { name: "Reaktor" },
      onAction,
    ...
    const swap = buttons.find(
      (button) => button.textContent === "Modul wechseln",
    );
    expect(swap.disabled).toBe(false);
    expect(buttons.filter((button) => button.disabled).length).toBe(
      buttons.length - 1,
    );
