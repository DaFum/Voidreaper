...
test("createMasteryChallenges generates exactly 5 challenges per item", () => {
  ...
  assert.equal(
    challenges.length,
    10,
    "Creates 5 challenges per ship and 5 per weapon",
  );

  const vesperChallenges = challenges.filter(
    (c) => c.category === "ship" && c.id.startsWith("mastery-vesper-"),
  );
  assert.equal(vesperChallenges.length, 5, "Creates 5 challenges for vesper");

  const railgunChallenges = challenges.filter(
    (c) => c.category === "weapon" && c.id.startsWith("mastery-railgun-"),
  );
...
test("createMasteryChallenges correctly configures mastery tiers and targets", () => {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    assert.equal(
      ...
      tier * 10,
      `Target should be ${tier * 10}`,
    );
    assert.equal(
      challenge.reward.challengeSeals,
      tier,
      `Reward should be ${tier} seals`,
    );
  }
});
