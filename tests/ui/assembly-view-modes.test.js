...
test("structure labels derive depth through parent nodes when no parent port remains", () => {
  ...
  const overlay = getViewModeOverlay(ASSEMBLY_VIEW_MODES.STRUCTURE, {
    assembly,
    geometry,
  });
...
test("diagnostic labels receive deterministic non-overlapping positions", () => {
  ...
  assert.ok(
    Math.sqrt(
      labels[1].position.x * labels[1].position.x +
        labels[1].position.y * labels[1].position.y,
    ) >= 12,
  );
  assert.deepEqual(
    layoutOverlayLabels(labels, { minimumDistance: 12 }),
    labels,
  );
});
...
test("connection overlays draw paths and labels", () => {
  ...
  const context = new Proxy(
    {},
    {
      get: (_target, property) =>
        ["save", "restore", "beginPath", "stroke"].includes(property)
          ? () => calls.push([property])
          : property === "moveTo" ||
              property === "lineTo" ||
              property === "fillText"
  ...
  assert.ok(
    calls.some(([name, x, y]) => name === "moveTo" && x === 1 && y === 2),
  );
  assert.ok(
    calls.some(([name, text]) => name === "fillText" && text === "standard"),
  );
