...
import {
  createDetachedDebris,
  updateDetachedDebris,
} from "../../../src/render/ship-assembly/detached-debris-renderer.js";

test("createDetachedDebris initializes with correct properties", () => {
  const geometry = { size: 10 };
  const debris = createDetachedDebris(geometry, {
    x: 10,
    y: 20,
    vx: 5,
    vy: -5,
...
test("updateDetachedDebris correctly updates pool and removes old debris", () => {
  ...
  assert.equal(
    updatedPool.length,
    1,
    "Should filter out debris older than lifetime",
  );
  ...
  assert.equal(
    debris.rotation,
    0.8,
    "Rotation should update based on spin and dt",
  );
});
