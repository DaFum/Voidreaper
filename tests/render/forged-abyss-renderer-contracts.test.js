...
} from "../../src/render/ship-assembly/adaptive-armor-renderer.js";
  ...
  const gradient = {
    addColorStop(offset, color) {
      assert.ok(
        ...
        `color stop offset out of range: ${offset}`,
      );
      assert.equal(
        typeof color,
        "string",
        `color stop is not a string: ${color}`,
      );
      assert.ok(
        !color.includes("NaN") && color !== "undefined",
        `malformed color stop: ${color}`,
      );
    },
  };
  const state = {
  ...
  return new Proxy(state, {
      ...
      if (key === "createLinearGradient" || key === "createRadialGradient")
        return (...args) => {
          for (const arg of args)
            assert.ok(
              Number.isFinite(arg),
              `non-finite ${key} coordinate: ${args}`,
            );
          hooks[key]?.(...args);
          return gradient;
        };
      ...
      return (...args) => {
        for (const arg of args)
          assert.ok(
            ...
            `non-finite argument to ${key}: ${args}`,
          );
        hooks[key]?.(...args);
      };
    },
    set(target, key, value) {
      if (typeof value === "string")
        assert.ok(
          !value.includes("NaN") && value !== "undefined",
          `malformed ${key}: ${value}`,
        );
      target[key] = value;
...
test("renderAdaptiveArmor draws every plate family at every LOD", () => {
  for (const lod of LODS)
    renderAdaptiveArmor(createStrictContext(), PLATES, PALETTE, { lod });
...
test("renderBalanceDecorators draws every decorator kind", () => {
  renderBalanceDecorators(
    ...
    [
      ...
      { position: { x: 5, y: 6 }, rotation: 1, scale: 0.8, kind: "fin" },
    ],
    PALETTE,
  );
});
...
test("renderConnector draws every layer at every LOD", () => {
  for (const lod of LODS)
    for (const layer of ["static", "dynamic", "all"])
      renderConnector(createStrictContext(), CONNECTOR, PALETTE, {
        lod,
        layer,
        energyFlow: 1.3,
      });
...
test("connector shading follows the connector, not the ship origin", () => {
  ...
  const ctx = createStrictContext({
    createLinearGradient: (...args) => coordinates.push(args),
  });
  renderConnector(ctx, CONNECTOR, PALETTE, { layer: "static" });
  assert.ok(
    coordinates.length >= 3,
    "expected gradients for the spine and both rails",
  );
  ...
  const anchors = [
    midpoint(CONNECTOR.spine),
    midpoint(CONNECTOR.leftRail),
    midpoint(CONNECTOR.rightRail),
    [CONNECTOR.spine.from.x, CONNECTOR.spine.from.y],
    [CONNECTOR.spine.to.x, CONNECTOR.spine.to.y],
  ];
  for (const [x0, y0, x1, y1] of coordinates) {
    const centre = [(x0 + x1) / 2, (y0 + y1) / 2];
    assert.ok(
      anchors.some(
        ([ax, ay]) => Math.hypot(ax - centre[0], ay - centre[1]) < 1e-6,
      ),
  ...
  for (const [type, points] of Object.entries(ENEMY_POINTS)) {
    ...
    for (let offset = 0; offset < 12; offset += 1) {
      ...
      const ctx = createStrictContext({
        ...
        lineTo: (x, y) => {
          if (cursor) path.push([cursor, [x, y]]);
          cursor = [x, y];
        },
        ...
        stroke: () => {
          if (
            lineWidth === 1.2 &&
            String(strokeStyle).startsWith("rgba(233,255,255")
          )
            segments.push(...path);
        },
        __set: (key, value) => {
    ...
    if (points >= 5)
      assert.ok(drawn > 0, `${type}: expected rim segments, captured none`);
    total += drawn;
  }
  assert.ok(
    total > 0,
    "captured no rim segments at all — the capture hook is broken",
  );
});
