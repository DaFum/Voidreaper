...
test("createBloomPass exits early if document is undefined", () => {
  ...
  assert.equal(
    ...
    0,
    "Does not apply bloom if no DOM available",
  );
});
...
test("createBloomPass exits early if canvas is missing or invalid", () => {
  ...
  try {
    ...
    assert.equal(
      ctxNoCanvas.save.mock.calls.length,
      0,
      "Exits if no canvas on ctx",
    );

    const ctxInvalidCanvas = {
      canvas: { width: 0, height: 100 },
      save: test.mock.fn(),
    };
    bloom.apply(ctxInvalidCanvas);
    assert.equal(
      ctxInvalidCanvas.save.mock.calls.length,
      0,
      "Exits if canvas has 0 width",
    );
  } finally {
