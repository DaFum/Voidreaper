...
## 2024-05-18 - Reroll & Banish Button Accessibility
**Learning:** Legacy UI elements that depend on `disabled` attributes often rely on parent container elements for tooltips (`parentElement.title`), which breaks accessibility because screen readers do not associate the wrapper's title with the disabled button.
**Action:** When updating disabled button UI patterns in legacy components (like `legacy-runtime.js`), ensure that disabled buttons self-document why they are disabled by adding descriptive `aria-label` and `title` attributes directly to the button, and including a visible, visually hidden span or small tag directly inside the button text with the reason.
