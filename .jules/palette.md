...
## 2026-09-07 - Explicit Disabled States for Economy Upgrades
**Learning:** Native `disabled` buttons in economy screens (like Hangar upgrades) lack context on *why* they are disabled (maxed out vs insufficient funds). Without `aria-label` or visible reasons, users may think the system is broken rather than understanding they need more resources.
**Action:** When disabling interactive elements based on dynamic constraints (like currency), always determine the specific reason, add semantic `title` and `aria-label` attributes, and embed a visible `<small aria-hidden="true">(reason)</small>` for sighted/touch users directly into the button.
